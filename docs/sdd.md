...
KD-003 Module integration through datasets. Modules integrate through shared datasets and schemas so ownership and boundaries remain explicit and reviewable, while Go modules may import other BusDK Go libraries for shared mechanics instead of invoking CLIs.
KD-004 Library-first Go modules. Go modules are structured around a library package that implements business logic, with the CLI as a thin wrapper for argument parsing and output formatting, so behavior is reusable across modules and test coverage can focus on library APIs.
