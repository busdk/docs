...
**Completeness:** 80% — init (incl. --dry-run), validate (incl. rate check, vat_registered=false, --period, --source journal), report and export from invoice or journal with source_refs, vat-returns index update, closed-period re-export/--force, and path API verified by e2e and unit tests; user can complete close-step VAT from either source.
