...
**Completeness:** 80% — init, add (header/lines), validate (global and per-invoice, total_net and total_vat), list with all filters, and postings verified by e2e and unit tests; add/add-line refuse on validation failure and dry-run verified; user can complete record-invoices and emit posting dataset for [bus-journal](./bus-journal); pdf not verified in this repo’s e2e.

**Use case readiness:** Accounting workflow: 80% — init, add, validate (totals and VAT), list with all filters, postings verified; user can complete record-invoices and feed journal; pdf not verified. Sale invoicing (sending to customers): 80% — same; pdf step blocked by [bus-pdf](./bus-pdf). Finnish company reorganisation: 80% — init, add, validate, list, postings support evidence-pack baseline; pdf not in journey.

**Current:** Verified only. E2E `tests/e2e_bus_invoices.sh`: help, version, usage exit 2, validate when missing/after init, init (eight files in workspace root only, no `invoices/`), init --dry-run, list TSV and filters (--type, --status, --month, --from/--to, --due-from/--due-to, --counterparty, --invoice-id), add (header) and `<invoice-id> add` (line), `<invoice-id> validate`, total_net and total_vat validation and add/add-line refuse when validation fails, add --dry-run, postings (invoice-postings.csv and schema, --dry-run), global flags (--output, --chdir, --quiet, --, --format, --color); pdf runs but e2e expects non-zero (bus-pdf not in PATH). Unit: `cmd/bus-invoices/run_test.go` (init, list filters, add, invoice-id add/validate, flags, usage, quiet/verbose/color/format/chdir/output, postings success/dry-run/validation-fails); `internal/initarea/initarea_test.go`; `internal/validate/*` (schema, types, CSV, foreign keys, list, validate_invoice totals/due-before-issue/total_vat); `internal/add/add_test.go`; `internal/cli/flags_test.go`, `help_test.go`, `color_test.go`; `internal/pdf/pdf_test.go` (BuildInvoiceRenderModel, RunBusPDF); `internal/posting/posting_test.go`; `paths/paths_test.go`.

**Planned next:** attachment_id in header schema and validation (FR-005, Finnish compliance) per PLAN.md; add postings to subcommand help so `bus invoices postings --help` shows subcommand-specific help. Advances accounting and evidence-pack use cases.

**Blockers:** [bus-pdf](./bus-pdf) required for `bus invoices pdf` to complete sale-invoicing PDF step. None for init/add/validate/list/postings.
