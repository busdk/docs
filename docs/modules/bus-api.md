...
- **Package:** `GET {base}/package` returns `datapackage.json` when present (404 otherwise).
- **Schema:** `GET {base}/resources/{name}/schema` returns the Table Schema for a resource.
- **Rows:** `GET {base}/resources/{name}/rows` supports query parameters such as `key`, `filter`, `column`, and `include_formula_source`. Single-row read uses `GET {base}/resources/{name}/row/{pk}` where `{pk}` is the percent-encoded JSON array of primary key values in schema order.
- **Mutate:** `POST {base}/resources/{name}/rows`, `PATCH {base}/resources/{name}/row/{pk}`, and `DELETE {base}/resources/{name}/row/{pk}` perform row add, update, and delete subject to schema constraints and update/delete policies. Schema and package mutation endpoints (e.g. `PATCH .../schema`, `PATCH .../package`) delegate to bus-data semantics. When `--read-only` is in effect, all mutating endpoints return 403.
