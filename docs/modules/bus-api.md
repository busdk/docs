...
For built-in adapters, module resource discovery is explicit and library-based. Dataset modules resolve paths through their Go path APIs (including reports through `bus-reports/path`); modules without workspace datasets return empty module resource lists.
