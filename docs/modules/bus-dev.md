...
The active runtime is chosen as follows. The `--agent <runtime>` flag for that invocation overrides everything else. If no flag is given, the session-stored preference **`BUS_DEV_AGENT`** applies when set (e.g. `export BUS_DEV_AGENT=codex`); that value is used as the default for every `bus dev` command in that session until the variable is unset or changed. When neither flag nor session preference is set, the [bus-agent](../sdd/bus-agent) library reads the **persistent default agent** from bus configuration (via the [bus-config](../sdd/bus-config) library); you can set it with [bus config set agent](./bus-config) (e.g. `bus config set agent gemini`) and it is saved in bus configuration. When no persistent default is set, the tool uses the automatic default from the set of available runtimes in the effective order (alphabetic by runtime ID by default). You can configure which agent is used first by setting an agent order, and you can disable or enable specific agents; see the [bus-agent](../sdd/bus-agent) SDD and [bus-agent module docs](./bus-agent) for the exact environment variables or config.
