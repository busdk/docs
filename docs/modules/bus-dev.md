...
`bus dev work <start|drain|list|status|stats|next|show|watch|wait|say|reopen|approve|close|fail|block|cancel> ...` — interactive controller for parallel development task streams over Bus Events. `drain` repeatedly dispatches modules with unchecked `PLAN.md` items until they are drained or progress stops.
...
its own `HEAD`. `--worktree-id <id> --reuse-worktree` tells worker-backed task
runners to reuse a preinitialized deterministic task worktree; the alias
`--preinitialized-worktree` has the same meaning.
...
**`e2e`** — Guided workflow to detect missing end-to-end tests for the current module and scaffold them in a hermetic way, consistent with BusDK testing conventions and the module’s implementation design spec and end-user documentation. May be combined with **plan**, **spec**, **work**, and **triage** in any order; operations run in the order you list them and stop on first failure. E2E tests live in the module-standard `tests/e2e.sh` harness. The tool uses the module’s design specs and end-user docs to determine which tests are needed and produces scaffolds that cover the behavior described there. When `PLAN.md` exists at repository root, the workflow reads it first, treats checked items as completed-feature coverage obligations, verifies each checked item is fully covered by e2e tests, and prioritizes unchecked test-related items. It also continues to search implementation design specs and end-user docs for other untested behavior exactly as it does without `PLAN.md`. This subcommand does not remove already checked plan items; checked-item pruning is handled by `bus dev plan`. The command does not perform remote Git operations or modify workspace accounting datasets.

**`triage`** — Keep development-state documentation accurate, compact, and evidence-based by reconciling what users can actually do (as proven by tests) with what is planned next (PLAN.md) and what depends on what. Updates **only** documentation: the overall [development-status](../modules/features) page and each module’s end-user docs page (the “Development state” section). Never modifies source code or test files. The command detects the project context automatically: run from the **docs repo** to update the development-status page and all module pages; run from the **BusDK super-project** to update the docs submodule’s same targets; run from a **single module repo** to update only that module’s page and the overall development-status page in the sibling docs repo. If the context is ambiguous, the command exits with code 2 and guidance on where to run it. May be combined with **plan**, **spec**, **work**, **e2e**, **stage**, and **commit** in any order (e.g. `bus dev triage plan work`); operations run in the order you list them and stop on first failure. At the start of the triage step, the tool prints to stderr which agent runtime and model are in use. See the [module reference](../modules/bus-dev) for the normative behavior, project-context detection rules, and the structure of the “Development state” sections triage writes.

**`each [--check] [--only MODULE[,MODULE...]]... [--skip MODULE[,MODULE...]]... [--jobs N|-j N] TOKEN...`** — Superproject-only helper that dispatches the remaining command as `bus dev TOKEN...` in each selected child module. Child discovery is deterministic and sourced from a deduplicated union of `.gitmodules` paths and top-level directories containing `.bus/dev`; if `bus` is present it runs first, then remaining discovered child paths are sorted lexicographically. A discovered child is selected when its directory contains a `Makefile` or `.bus/dev`. Before any module command runs, `each` preflights every selected module with the same tokens and fails immediately if any module cannot resolve the token sequence or has a non-runnable script action (Unix script actions require `.sh` with `+x`). With `--check`, only preflight is run and no module command is executed. Use `--only` to run exactly a named module set (`--only` can be repeated and accepts comma-separated names). Use `--skip` to exclude modules by directory name from that selected set or from the full discovered set (`--skip` can be repeated and accepts comma-separated names). Use `--jobs` / `-j` to run up to N child modules at a time; when omitted, the default is the processor count capped at 8. Inside each child module the token sequence still runs in order, so `stage commit` stays `stage` then `commit` for that module. `each` itself is not a workflow step and is not expanded by pipelines. Example: `bus dev each --only bus,bus-api stage commit` runs `bus dev stage commit` only in the named child modules; `bus dev each --jobs 8 --skip bus-docs,bus-legacy stage commit` runs `bus dev stage commit` in each non-skipped child module with up to eight modules active at a time; `bus dev each --check --only bus,bus-api retry -f fix-tests test` preflights retry fallback/target resolution in exactly the selected modules.
...
`-h` and `--help` print help to stdout and exit 0. Help describes built-in operations and pipelines and points to `bus dev list`, `bus dev pipeline list`, `bus dev action list`, and `bus dev script list` for repository-local discovery. Operation-specific help is available both as `bus dev -h each` and as `bus dev each --help` so the matching help page is reachable from either position. `-V` and `--version` print tool name and version and exit 0.
...
| Key | Description |
|-----|-------------|
| `bus-dev.agent` | Bus-dev default agent runtime. Valid values: **`cursor`**, **`codex`**, **`codex:local`**, **`gemini`**, **`claude`**. Resolution order: `--agent` → `BUS_DEV_AGENT` → `BUS_AGENT` → `bus-dev.agent` → `bus-agent.runtime`; no implicit fallback is chosen. Set with `bus dev set agent <runtime>`. For the shared default when not using bus-dev, set `bus-agent.runtime` via `bus agent set runtime <runtime>`. |
...
| `bus-dev.model_reasoning_effort` | Bus-dev default model reasoning effort (Codex values: `minimal`, `low`, `medium`, `high`, `xhigh`). Resolution order: `BUS_DEV_MODEL_REASONING_EFFORT` → `bus-dev.model_reasoning_effort` → `BUS_AGENT_MODEL_REASONING_EFFORT` → `bus-agent.model_reasoning_effort`. Set with `bus dev set model-reasoning-effort <value>`. |
| `bus-dev.model_verbosity` | Bus-dev default model verbosity (Codex values: `low`, `medium`, `high`). Resolution order: `BUS_DEV_MODEL_VERBOSITY` → `bus-dev.model_verbosity` → `BUS_AGENT_MODEL_VERBOSITY` → `bus-agent.model_verbosity`. Set with `bus dev set model-verbosity <value>`. |
...
| `bus-dev.model_reasoning_effort_for.<selector>` | Bus-dev per-step model reasoning effort. Same selector semantics as `agent_for`. Set with `bus dev set model-reasoning-effort-for <token|@token> <value>`. |
| `bus-dev.model_verbosity_for.<selector>` | Bus-dev per-step model verbosity. Same selector semantics as `agent_for`. Set with `bus dev set model-verbosity-for <token|@token> <value>`. |
| `bus-dev.model_reasoning_effort_for_model.<model>` | Bus-dev per-model reasoning effort override. Applied when resolved model equals `<model>`. Set with `bus dev set model-reasoning-effort-for-model <model> <value>`. |
| `bus-dev.model_verbosity_for_model.<model>` | Bus-dev per-model verbosity override. Applied when resolved model equals `<model>`. Set with `bus dev set model-verbosity-for-model <model> <value>`. |
| `bus-dev.env_for.<selector>.<env_name>` | Bus-dev per-step environment override. Selector semantics match `agent_for` (`@token` first then `token` for built-in steps, `token` only for user-defined). Set with `bus dev set env-for <token|@token> <ENV_NAME> <value>`. |
| `bus-dev.allowed_dirs_for.<selector>` | Bus-dev per-step allowed directories. Selector semantics match `agent_for` (`@token` first then `token` for built-in steps, `token` only for user-defined). Values are JSON arrays of directory strings. Set with `bus dev set allow-dirs-for <token|@token> <dir...>`. |
| `bus-dev.allowed_dirs_for_module.<module>.<selector>` | Bus-dev per-module per-step allowed directories. Applied only when repository module name equals `<module>` and layered after `bus-dev.allowed_dirs_for.*` for matching selector. Set with `bus dev set allow-dirs-for-module <module> <token|@token> <dir...>`. |
...
**Option: refresh development-state docs.** To keep the [development-status](../modules/features) page and each module’s “Development state” section accurate and evidence-based, run `bus dev triage` from the docs repo, the BusDK super-project, or a single module repo (the tool detects the context). You can chain it with other workflows, for example `bus dev triage plan work` to update docs then plan and work in a module.
...
bus dev each --check --jobs 8 stage commit
...
bus dev each --check --skip bus-docs --jobs 8 stage commit
bus dev each --jobs 8 --skip bus-docs,bus-legacy stage commit
...
- [Implementation: Module repository structure and dependency rules](../architecture/independent-modules)
