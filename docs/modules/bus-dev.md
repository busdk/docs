...
Subcommands that invoke an external agent (commit, work, spec, and when used during init or e2e) use the [bus-agent](../sdd/bus-agent) library and one of its supported runtimes: **Cursor CLI**, **Codex**, **Gemini CLI**, and **Claude CLI**. The active runtime is chosen as follows. The `--agent <runtime>` flag for that invocation overrides everything else. If no flag is given, the session-stored preference **`BUS_DEV_AGENT`** applies when set (e.g. `export BUS_DEV_AGENT=codex`); that value is used as the default for every `bus dev` command in that session until the variable is unset or changed. When neither flag nor session preference is set, the tool uses the automatic default: it detects which agent CLIs exist in PATH (enabled agents) and picks one in a documented, deterministic preference order. An agent is enabled if and only if its CLI command exists at resolution time.

Invalid runtime names (e.g. `--agent unknown` or an invalid value in `BUS_DEV_AGENT`) produce a clear usage error and exit 2. If the user has selected an agent (via flag or `BUS_DEV_AGENT`) and that agent’s CLI is not installed or not in PATH, the tool reports that on stderr, directs you to the canonical installation URL for that runtime, and exits with code 1. When no runtime is selected and no agent CLI is found in PATH (no enabled agent), the tool exits with a clear diagnostic and directs you to install at least one supported agent, with pointers to the canonical installation URLs for each runtime. Model, output format, and timeout for the agent are configurable via the runtime’s own environment variables or flags (documented by each runtime).
