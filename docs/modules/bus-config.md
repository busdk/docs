...
The workspace file holds [accounting entity](../master-data/accounting-entity/index) settings (company name, Y-tunnus business ID, legal-form profile, base currency, fiscal year boundaries, VAT registration, VAT cadence/timing, VAT command defaults, optional VAT registration dates, and Finnish statutory reporting profile keys). Other modules read these values instead of duplicating settings in row-level datasets.
