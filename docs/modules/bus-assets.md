...
**Completeness:** 70% — init (incl. partial-init failure FR-INIT-004), add, validate, schedule, depreciate, dispose, post and path accessors (NFR-AST-002) verified by tests; user can complete full asset lifecycle and produce postings for evidence pack; FR-AST-003 (cap at disposal), FR-AST-004 (no dep row when fully depreciated) and require --gain-account/--loss-account on dispose not test-covered.

**Use case readiness:** Finnish company reorganisation (yrityssaneeraus): 70% — full lifecycle and postings verified; FR-AST-003, FR-AST-004 and dispose required-args not test-covered.

**Current:** init verified by `cmd/bus-assets/run_test.go` (TestInitCreatesDataset, TestInitFailsOnPartialState) and `tests/e2e_bus_assets.sh`. add verified by `run_test.go` (TestAddAcceptsMethodAliasStraightLine, TestAddRejectsInvalidMethod) and e2e. validate, schedule, post verified by `run_test.go`, `run_property_test.go`, `internal/assets/schedule_property_test.go`, `post_property_test.go`, `period_property_test.go`, `load_assets_property_test.go`, and e2e. depreciate verified by `run_test.go` (TestDepreciateMatchesPostOutput) and e2e. dispose verified by `run_test.go` (TestDryRunDisposePrintsSummary, TestDisposeFailsWhenValidationFails) and e2e. Path accessors verified by `internal/assets/paths_test.go` (TestRegisterPathAndSchemaPath). Workspace-root layout (no `assets/` subdir) verified by e2e. Global flags verified by `internal/cli/flags_test.go`, `flags_property_test.go` and e2e.

**Planned next:** Cap accumulated depreciation to depreciable base in disposal postings (FR-AST-003); require `--gain-account` and `--loss-account` on dispose with exit 2 when missing (PLAN.md); add test that disposal produces no depreciation row for disposal month when asset already fully depreciated (FR-AST-004). Advances Finnish company reorganisation evidence pack.
