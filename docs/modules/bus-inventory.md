...
The `items.csv` and `movements.csv` datasets can resolve either as ordinary CSV or as `PCSV-1` fixed-block CSV through workspace storage metadata.
...
Inventory item and movement datasets and their beside-the-table schemas live in the inventory area. Master data for this module is stored in the workspace root only; the module does not use subdirectories (for example, no `inventory/` folder). Other tools that need these paths use the module API described in the [Data path contract](../modules/index#data-path-contract-for-read-only-cross-module-access).

When `datapackage.json` selects `PCSV-1` for one or both inventory tables, `init`, `add`, `move`, `status`, `valuation`, and `validate` all use the shared storage-aware table layer from `bus-data`. The baseline schemas written by `init` become `PCSV-1` compatible by adding a visible `_pad` field and the needed storage metadata. Plain CSV workspaces keep their current behavior.
