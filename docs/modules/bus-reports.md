...
`evidence-pack` is the one-command close bundle. It writes a target directory full of standard artifacts and also writes a manifest of what it created. The package now includes both `materials-register` and `methods-description` as first-class PDFs, a `bank-control` / statement-continuity TSV, `annual-template` and `annual-validate` summaries, internal `tase-erittelyt`, and explicit compact/full/account-breakdown statutory PDFs alongside the main statements, ledgers, and close manifests. You can trim the package with `--profile accountant|machine` or explicit `--include` / `--exclude` selectors.
