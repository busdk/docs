...
**Completeness:** 80% — Init, open, list, validate, close, lock, and opening (year rollover from prior workspace) verified by e2e and unit tests; merge-conflict and non-Git hint verified. E2E for opening `--replace` and `--allow-as-of-mismatch` (PLAN.md) would raise to 90.

**Use case readiness:** [Accounting workflow](../workflow/accounting-workflow-overview): 80% — init, open, list, validate, close, lock, opening verified; user can complete period open/close/lock and year-rollover opening. [Finnish bookkeeping and tax-audit compliance](../compliance/fi-bookkeeping-and-tax-audit): 80% — close, lock, opening with append-only and locked state verified. [Finnish company reorganisation](../compliance/fi-company-reorganisation-evidence-pack): 80% — close, lock, opening for snapshots verified. [Finnish payroll handling (monthly pay run)](../workflow/finnish-payroll-monthly-pay-run): 80% — period open/close/lock and opening for payroll month verified.
