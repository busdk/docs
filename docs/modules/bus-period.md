...
**Completeness:** 80% — Init, open, list, validate, close, lock, and opening (year rollover from prior workspace) verified by e2e and unit tests; merge-conflict detection and non-Git workspace hint verified in e2e. E2E for opening `--replace` and `--allow-as-of-mismatch` (PLAN.md) would raise to 90.

**Use case readiness:** [Accounting workflow](../workflow/accounting-workflow-overview): 80% — init, open, list, validate, close, lock, opening verified; user can complete period open/close/lock and year-rollover opening; E2E for --replace and --allow-as-of-mismatch planned. [Finnish bookkeeping and tax-audit compliance](../compliance/fi-bookkeeping-and-tax-audit): 80% — close, lock, and opening with append-only and locked state verified. [Finnish company reorganisation](../compliance/fi-company-reorganisation-evidence-pack): 80% — close, lock, and opening for snapshots verified. [Finnish payroll handling (monthly pay run)](../workflow/finnish-payroll-monthly-pay-run): 80% — period open/close/lock and opening for payroll month verified.

**Current:** E2e `tests/e2e_bus_period.sh` proves init (root, idempotent), list, validate (success, unbalanced/schema-missing/merge-conflict fail), close (--period required, dry-run, append-only, close_entries/opening_balances, journal-closed-periods.csv, refuses unbalanced or already closed), lock (--period, fails on open period, closed→locked), opening (prior→current workspace, required flags, already-exists without --replace, dry-run), merge-conflict detection (validate with conflict markers), non-Git workspace hint (-v list), and global flags (help, version, color, output, quiet, chdir, invalid color/format, quiet+verbose, --, -vv, stderr). Unit tests: `internal/app/run_test.go` (open success/--period/positional reject, init/close/lock/validate/list/opening, dry-run, chdir, output, quiet, invalid usage); `internal/period/period_test.go` (List, Init, Open marks open/not-found, init idempotent/inconsistent); `internal/period/opening_test.go` (BalancesAsOf, LoadPriorWorkspace, opening logic); `internal/period/periodid_test.go`, `internal/period/closed_periods_file_test.go`; `internal/validate/validate_test.go`; `internal/cli/flags_test.go`.

**Planned next:** E2E for opening `--replace` (remove only deterministically identified opening rows, then create new entry) and `--allow-as-of-mismatch` (prior workspace fiscal year end vs --as-of) per PLAN.md. Advances [Accounting workflow](../workflow/accounting-workflow-overview).
