...
Most BusDK module CLIs accept a common set of global flags before the subcommand. These flags behave the same across modules that support them: help and version exit immediately, working directory and output redirection are applied before any module logic, and color can affect either diagnostic text or module-defined row text when explicitly documented. In module synopsis lines this set is referred to as **[global flags]**; the list below is the canonical definition.
...
- **`--color <mode>`** — Control colored output for human-facing messages, and some module-specific result rows. `<mode>` must be one of `auto`, `always`, or `never`. `auto` enables color when output streams are terminal-capable; `always` forces color; `never` disables color. An invalid mode is invalid usage and exits with status 2. The `NO_COLOR` environment variable (any non-empty value) suppresses color only in `auto` mode; it does not override an explicit `--color=always`.
- **`--no-color`** — Alias for `--color=never`. `--no-color`/an explicit `never` mode wins over an explicit `--color=always`, regardless of which is given first.
