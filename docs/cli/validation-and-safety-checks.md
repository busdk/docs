...
Mutation commands perform schema and logical validation before writing workspace data. Read-only report commands validate their inputs before producing a report but do not mutate data. Schema validation ensures type correctness and referential integrity. Logical validation enforces business rules such as existing account references, balanced debits and credits for transactions, invoice totals matching line items, and VAT classification completeness for VAT reports. If errors are found, the command fails with a clear diagnostic and refuses to commit invalid data.
...
For migration-quality controls, the operator explicitly invokes deterministic parity and gap checks between source imports and workspace or journal activity; these checks are not an implicit validation step for every command. The command flow is documented in [Source import parity and journal gap checks](../workflow/source-import-parity-and-journal-gap-checks).

<!-- busdk-docs-nav start -->
<p class="busdk-prev-next">
  <span class="busdk-prev-next-item busdk-prev">&larr; <a href="./output-style">CLI output style</a></span>
