...
When an end-to-end suite depends on a host capability such as loopback TCP bind, browser headless support, or another strictly local runtime feature, the suite should probe that capability once up front and print an explicit `SKIP <test>: <reason>` line for only the affected cases when the environment does not allow it. Generic failures for missing host capabilities make root-cause analysis slower than necessary and should be avoided.
