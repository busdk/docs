...
|---|---|---|---|---|
| core-1 | `pkg/ui/navigation_primitives.go`, `pkg/ui/navigation_primitives_gx_adapter.go`, `pkg/ui/navigation_primitives_gx_generated.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go` | `pkg/uikit/navigation_primitives.go`, `pkg/uikit/navigation_primitives_gx_adapter.go`, `pkg/uikit/navigation_primitives_gx_generated.go`, `pkg/uikit/navigation_primitives_test.go` | menu, tabs, navigation props/events/errors, href validation, dispatch helpers, `DOMAttrUIValue`, compiled GX markup and legacy trigger-HTML splice behavior | implementation-ready via source map `logs/worker-prompts/gx-ui-core-ui-navigation-primitives-source-map-20260616a.md`; semi-mechanical patch skeleton plus review; remove menu/tabs/navigation aliases and pass focused tests plus no-`uikit` audit |
...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/navigation_primitives.go`, `pkg/ui/navigation_primitives_gx_adapter.go`, `pkg/ui/navigation_primitives_gx_generated.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, focused tests | `Menu*`, `Tabs*`, `Navigation*`, navigation events/errors still routed through `pkg/uikit` | shell/navigation child: menu/tabs/navigation ownership | move `MenuItem*`, `MenuProps`, `TabsProps`, `NavigationProps`, click events/handlers, checked/node helpers, dispatch helpers, `ValidateNavigationHref`, `DOMAttrUIValue`, compiled GX helpers, and navigation validation errors from `pkg/uikit/navigation_primitives*.go` into `pkg/ui` | implementation-ready / source-map complete | Source-map/prompt artifact `logs/worker-prompts/gx-ui-core-ui-navigation-primitives-source-map-20260616a.md` names exact files, symbols, behavior invariants, checks, and automation classification. DoD includes facade parity in `pkg/ui/ui_test.go`, focused dispatch/href/splice validation tests, and scoped audit for `uikit.Menu|uikit.Tabs|uikit.Navigation|uikit.ValidateNavigationHref`. |
