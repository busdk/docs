...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/portal_launcher_empty_state.go`, likely `pkg/ui/shell_navigation_status.go` and `pkg/ui/ui.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` portal launcher/status surface facade still backed by `uikit` status aliases | move portal launcher empty-state rendering onto package-owned `pkg/ui` empty-state/status surface helpers first; then rerun deletion probe to name whether broader loading/result/error/shell aliases are next | active / split required | New first compiler-derived blocker from the post-control-bridge hydrated deletion/build-exclusion probe on `bus-ui` `4089eae`: `pkg/ui/portal_launcher_empty_state.go:12:2: no required module provides package github.com/busdk/bus-ui/pkg/uikit`. Supervisor source map shows the first child is empty-state ownership: `EmptyStateProps`, `EmptyStateNodeChecked`, `EmptyStateChecked`, `EmptyState`, and the portal launcher wrapper can move using already-owned `pkg/ui` status helpers from `data_evidence_provider_error.go`; broader shell/navigation/loading/result/error aliases remain out of scope until the next deletion probe names them. Do not unpark adopters until this row lands and the next deletion probe advances. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/portal_launcher_empty_state.go`, `pkg/ui/status_empty_state.go`, `pkg/ui/shell_navigation_status.go`, focused tests | `EmptyStateProps`, `EmptyStateNodeChecked`, and portal empty-state render still routed through `pkg/uikit` | portal/status child: empty-state ownership | move empty-state props/render helpers into `pkg/ui`, update `PortalLauncherEmptyState` to call local helpers, remove only empty-state aliases from `shell_navigation_status.go`, and add focused render/validation/action tests | queued | DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped audit proving no `pkg/uikit|uikit\.` in portal/status-empty-state files, and no empty-state `uikit` aliases remain in scoped facade files; deletion probe follows only after acceptance |
