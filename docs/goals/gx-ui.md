...
- known remaining core production rows: 0; the final audit/probe row below is
...
- known adopter-discovered public facade gaps: 0 open; `core-20`, the ledger
  WASM projection/render/controller helper gap, is accepted after `core-20a`,
  `core-20b`, and the ledger WASM public-selector rewrite proved no remaining
  production public-facade gap. `core-19`, `core-15`, the factory
...
Current deletion sequencing result: `bus-ui/pkg/uikit` and `bus-ui/pkg/uikit/uikittest` are deleted in the actual `bus-ui` tree. After deletion, `go list ./pkg/...` lists only public packages (`assistantui`, `terminalruntime`, `terminalui`, `ui`, `uiartifact`, `uicatalog`, `uiportal`, `uitest`). Verification passed with low-space cache settings: `go test -vet=off -p 1 ./pkg/... ./examples/...` from `bus-ui`. The compact static inventory reports 0 owner facade production files, 0 production adopter/core files, and 0 refined Go test files;
...
| core-14 | `pkg/ui` owner audit and `pkg/uikit`/`pkg/uikit/uikittest` deletion | all accepted replacement files | no production `pkg/ui` backing import/call to `pkg/uikit`; old package is not retained as compatibility layer | accepted / deleted in `bus-ui`; actual `pkg/uikit` and `pkg/uikit/uikittest` trees are gone after production, adopter, test, and example cleanup. `go list ./pkg/...` lists only public packages, `go test -vet=off -p 1 ./pkg/... ./examples/...` passes, and compact inventory reports 0 production and 0 test hits. |
| core-15 | `pkg/ui/ai_runtime_js.go`, `pkg/ui/ai_callback_registry_js.go`, `pkg/ui/app_scaffold_js.go`, `pkg/ui/runtime_helpers.go`, `pkg/ui/logger.go`, `pkg/ui/logger_js.go`, `pkg/ui/ai_api_client.go`, `pkg/ui/ai_refresh.go`, `pkg/ui/ai_refresh_callbacks.go`, `pkg/ui/ai_refresh_host_state.go`, `pkg/ui/ai_refresh_log*.go`, `pkg/ui/error_host*.go`, `pkg/ui/error_reporter_js.go`, `pkg/ui/ai_drop_decode.go`, focused native and js/wasm tests | `pkg/uikit/ai_runtime_js.go`, `pkg/uikit/ai_callback_registry_js.go`, `pkg/uikit/app_scaffold_js.go`, `pkg/uikit/runtime_helpers.go`, `pkg/uikit/logger.go`, `pkg/uikit/logger_js.go`, `pkg/uikit/ai_api_client.go`, `pkg/uikit/ai_refresh.go`, `pkg/uikit/ai_refresh_callbacks.go`, `pkg/uikit/ai_refresh_host_state.go`, `pkg/uikit/ai_refresh_log*.go`, `pkg/uikit/error_host*.go`, `pkg/uikit/error_reporter_js.go`, `pkg/uikit/ai_drop_decode.go`; already-owned `pkg/ui` dependencies include `ErrorReporter`, `RecoverToReporter`, `HTTPGatewayClient`, `ViewMountOptions`, `MountHTMLWithScrollPreservation`, `Global*`, `SetClassOnSelector`, `ReadDroppedFileBytes`, `UploadFileMultipart`, `Dispose`, `ChainDisposers`, AI drop services, and assistant attachment/state DTOs | public WASM AI runtime/scaffold/callback registry/logging/error-host/refresh helpers needed by adopter WASM apps; no compatibility shell should remain | accepted / promoted in `bus-ui` `13ce982`; Spark worker `gx-ui-core-ui-runtime-scaffold-spark-20260616a` false-actived with no assistant response or diff, Mini worker `gx-ui-core-ui-runtime-scaffold-mini-20260616a` started but stalled after a path correction, so the reviewed worker-owned exception path produced and verified the patch. Checks passed in worker/primary as applicable: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, production scoped no-`uikit` audit for `pkg/ui` with only accepted `assets/uikit.css` literal, compact static inventory, and primary `GOCACHE=/private/tmp/bus-ui-gocache-core15 GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`. |
| core-16 | `pkg/ui/runtime_api_url.go`, `pkg/ui/runtime_api_url_test.go` | `pkg/uikit/runtime_config.go` | `PublicRuntimeAPIConfig` public runtime API config helper discovered while replacing `bus-factory/internal/serve/browser_runtime.go`; keep URL resolution and lower-kebab public config validation in `pkg/ui` instead of preserving a `uikit` import | accepted / promoted in `bus-ui` `e6b2a7d`; primary checks passed with writable cache: `go test ./pkg/ui`, `go test ./...`. This was an adopter-discovered public-facade gap and is now closed. |
| core-17 | `pkg/ui/assistant_shell_page.go`, `pkg/ui/assistant_shell_page_test.go` | `pkg/uikit/assistant_shell_page.go` and minimal slot helper behavior from `pkg/uikit/assistant_workbench.go` | `RenderAssistantShellPage` / `AssistantShellPageOptions` public shell-page renderer discovered while replacing `bus-factory/internal/serve/server.go`; keep the generated HTML shell and slot replacement behavior in `pkg/ui` as an explicit HTML boundary instead of preserving a `uikit` import | accepted / promoted in `bus-ui` `6856dfc`; primary checks passed with writable cache: `go test ./pkg/ui`, `go test ./...`. This was an adopter-discovered public-facade gap and is now closed. |
| core-18 | `pkg/assistantui/assistantui_ai_panel_render.go`, `pkg/assistantui/assistantui_ai_facade_test.go` | existing `pkg/assistantui` renderer plus action contract formerly visible through `pkg/uikit.RenderAIPanel` | preserve browser action hooks for assistant panel renderers (`new-thread`, `rename-thread`, `send`, `interrupt`) so adopters can move to public `assistantui.RenderAIPanelHTML` without losing the existing client-script contract | accepted / promoted in `bus-ui` `bbc560c`; primary checks passed with writable cache: `go test ./pkg/assistantui ./pkg/ui`, `go test ./...`. This was an adopter-discovered public-facade gap and is now closed. |
| core-19 | `pkg/ui/ai_action_controller_js.go`, `pkg/ui/ai_href_resolver.go`, `pkg/ui/ai_event_wiring_js.go`, `pkg/ui/ai_drop_paths_js.go`, `pkg/ui/lifecycle_window_js.go`, `pkg/ui/window_close_guard.go`, plus dependent AI action/thread/route/polling helpers and focused native/js-wasm tests | `pkg/uikit/ai_action_controller_js.go`, `pkg/uikit/ai_href_resolver.go`, `pkg/uikit/ai_event_wiring_js.go`, `pkg/uikit/ai_drop_paths_js.go`, `pkg/uikit/lifecycle_window_js.go`, `pkg/uikit/window_close_guard.go`, `pkg/uikit/ai_action_router.go`, `pkg/uikit/ai_thread_actions.go`, `pkg/uikit/ai_actions*.go`, `pkg/uikit/ai_approval_flow.go`, `pkg/uikit/ai_thread_rename_js.go`, `pkg/uikit/polling.go`, `pkg/uikit/ai_thread_activity.go`, `pkg/uikit/ledger_routes.go`; public assistant DTO/constant helpers reused from `pkg/assistantui` | public WASM app action/event/lifecycle helpers needed by ledger adopter-9a: `AIActionController`, `AIActionControllerOptions`, `BuildAIMessageHrefResolver`, `AIPanelEventWiringOptions`, `WireAIPanelEvents`, `CollectAIDroppedPaths`, `WindowCloseGuardState`, `WindowCloseLifecycleOptions`, `WireWindowCloseGuardLifecycle`, `BuildAIWindowCloseGuardState`, related close-attempt helpers, AI action lifecycle helpers, route/poll helpers, and draft/attachment/thread helpers. | accepted / promoted in `bus-ui` `61251ea`; Spark worker `gx-ui-core-ui-wasm-app-wiring-spark-20260616a` false-actived with no assistant response or diff, so the reviewed worker-owned exception path produced worker commit `f57d51d` and promoted it to primary. Checks passed in worker and primary: `go test ./pkg/ui`, `go test ./...`, `GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`, `git diff --check`, and scoped no-`uikit` audit for the new `pkg/ui` files. The failed adopter-9a ledger worker-tree patch was reverted cleanly before this core repair. |
| core-20 | parent row for ledger WASM projection/render/controller helper facades | `pkg/uikit` helpers named by post-`core-19` ledger WASM compile feedback: `MountedTextView`, `NewParagraphTextView`, `ParseSplitRouteSelection`, `RenderAIMessageFromLocation`, `BuildAIPanelRenderProps`, `MountAIPreservedView`, `FindSelectedLine`, `NormalizeProjectionListMode`, `ProjectionListModeButtonClasses`, and additional list/detail presenter helpers after the first ten errors | public helpers needed before ledger WASM adopter rows can move to `pkg/ui` types without package-mismatch failures. Do not patch ledger around these with `uikit` imports; source-map the helper family first, then split if independent. | accepted / matrix advanced after `core-20a` `bus-ui` `ed018a0`, `core-20b` `bus-ui` `04e6256`, and ledger WASM public-selector rewrite `bus-ledger` `769b334`. Fresh compact inventory reports 0 adopter-discovered public facade gaps and no `bus-ledger` production `pkg/uikit` hits. |
| core-20a | `pkg/ui/split_controller_js.go`, `pkg/ui/mounted_text_view_js.go`, `pkg/ui/view_mount_ai_js.go`, `pkg/ui/evidence_helpers.go`, focused native and js/wasm tests | `pkg/uikit/split_controller_js.go`, `pkg/uikit/mounted_text_view_js.go`, `pkg/uikit/view_mount_ai_js.go`, `pkg/uikit/evidence_helpers.go`; already-owned dependencies include `pkg/ui/ledger_routes.go`, `pkg/ui/subroute_js.go`, `pkg/ui/view_mount_js.go`, `pkg/ui/control_primitives.go`, `pkg/ui/ai_refresh_types.go`, and `pkg/assistantui` markdown/panel helpers | first slice for `SplitRouteSelection`, `ParseSplitRouteSelection`, `MountedTextView`, `NewMountedTextViewWithOps`, `NewParagraphTextView`, `MountAIPreservedView`, `FindSelectedLine`, `RenderAIMessageFromLocation`, and a `pkg/ui` forwarding boundary for `BuildAIPanelRenderProps` that uses canonical `pkg/assistantui` behavior rather than recopying it. Preserves nil-safety, route parsing, mounted text callbacks, AI scroll-preserved mount options, selected-line fallback lookup, and location-derived assistant markdown/message rendering. | accepted / promoted in `bus-ui` `ed018a0`. Spark workers `...20260616a` through `...20260616e` exposed dispatch/runtime issues: create-then-message had no active session, prompt-file creates could not read supervisor/BusDK prompt paths, and corrected inline `--module bus-ui` create materialized the module but failed because the local provider on `127.0.0.1:11434` refused connections. The reviewed worker-owned exception path produced worker commit `58755be` and primary commit `ed018a0`. Checks passed in worker and primary: `go test ./pkg/ui`, `go test ./...`, `GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`, `git diff --check`, scoped no-`uikit` audit for touched `pkg/ui` files, compact inventory, and existing ledger WASM compile proof with `GOOS=js GOARCH=wasm go test -vet=off -p 1 -exec=echo ./internal/ui/wasm` after local temp/cache cleanup. |
| core-20b | `pkg/ui/projection_list_panel_js.go`, `pkg/ui/projection_list_panel_js_test.go`, `pkg/ui/ai_panel_render.go`, `pkg/ui/ai_panel_render_test.go` | `pkg/uikit/projection_list_panel.go`; canonical `pkg/assistantui.RenderAIPanelHTML` renderer for the public AI panel HTML boundary | projection list presenter helpers `NormalizeProjectionListMode`, `ProjectionListHeaders`, `ProjectionListModeButtonClasses`, and public `ui.RenderAIPanel` wrapper without recopying assistant panel behavior | accepted / promoted in `bus-ui` `04e6256`. Spark worker `gx-ui-core-ui-projection-list-panel-spark-20260616a` materialized the module but failed because the local provider on `127.0.0.1:11434` refused connections, so the reviewed worker-owned exception path produced worker commit `50233cd` and primary commit `04e6256`. Checks passed in worker and primary with low-space cache variants as needed: `go test ./pkg/ui`, `go test ./...`, `GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`, `git diff --check`, scoped no-`uikit` audit for touched `pkg/ui` files, compact inventory, and ledger WASM compile proof after the public-selector rewrite. |
...
1. Close the final core audit/probe row by splitting the remaining deletion
   failures into test/example/adopter rows from the fresh production-clean
   owner-facade base.
...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/globals_js.go`, `wasm_dom_js.go`, `view_mount_js.go`, `api_url_js.go`, `subroute*.go`, deleted `pkg/ui/wasm_facade_js_wasm.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` WASM DOM/global facade formerly backed by `uikit` | source-map JS global access, DOM attribute helpers, click binding, closest element, API URL, subroute helpers, and mount-with-scroll preservation helpers; move to `pkg/ui` with JS/WASM checks | accepted / matrix advanced | Accepted in primary `bus-ui` `dd210ad`; checks passed: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped no-`uikit` audit for moved WASM/global files, compact static audit, and hydrated deletion/build-exclusion probe. Later AI-drop acceptance dropped static production audit to 1 owner facade file and 33 production adopter/core files. |
| `bus-ui` | `bus-ui` | deleted `bus-ui/pkg/ui/ai_drop_facade_js_wasm.go`; package-owned AI drop/import files under `pkg/ui` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` AI drop JS/WASM facade formerly backed by `uikit` | source-map AI drop controller, JS file reader, drop services, file-byte reader, multipart upload, import/drop-zone handlers, visual target helpers, and direct helper dependencies; move to `pkg/ui` with JS/WASM checks | accepted / matrix advanced | Accepted in primary `bus-ui` `dfc25a5`; checks passed: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped no-`uikit` audit for AI-drop files, compact static audit, and `GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`. Owner-package deletion/build-exclusion probe now advances to production blocker `pkg/ui/split_projection_facade_js_wasm.go:5:8` and native test-only residue `pkg/ui/split_projection_facade_test.go`. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/split_projection_facade.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` split projection/query facade formerly backed by `uikit` | source-map panel/split layout state, projection response DTOs, route policy, and generic projection query client; move to `pkg/ui` with native tests | accepted / matrix advanced | Accepted in primary `bus-ui` `cd4fc20`; checks passed: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped no-`uikit` audit for `pkg/ui/split_projection_facade.go`, and deletion probe advancement to later owner-facade rows. |
| `bus-ui` | `bus-ui` | deleted `bus-ui/pkg/ui/split_projection_facade_js_wasm.go`; package-owned split projection JS/WASM helper files under `pkg/ui` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` split projection JS/WASM facade formerly backed by `uikit` | source-map locale formatter, split resize state/wiring, projection line summary, projection detail presenter, and direct formatter/evidence helper dependencies; move to `pkg/ui` with js/wasm checks | accepted / matrix advanced | Accepted in primary `bus-ui` `cc4a0c4`; checks passed: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped no-`uikit` audit for split-projection files, compact static audit, and `GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui`. Static owner-facade production audit is now clean. |
...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/ui.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` common primitive facade formerly backed by `uikit` | split table-first by primitive family: common HTML helpers, component/render runtime aliases, CSS/icons, DOM element helpers, status/surface/card/split aliases, `RenderGXNode`, and generic `DispatchAction`; re-audit after each accepted core slice because other rows may drain part of this file | accepted / matrix advanced | Completed across core-1 through core-8; `pkg/ui/ui.go` no longer imports/calls `pkg/uikit`, and later owner-facade audit after core-13 reports 0 production `pkg/ui` facade files backed by `uikit`. |
...
| Factory server shell helpers | `bus-factory/internal/serve/server.go` | Accepted `pkg/ui` token/static/client-log/listener/CSS helpers and `assistantui` panel shell helpers. | Token URL/path suffixes, static/index fallback, client-log API, listener URL, AI panel script/render boundary. | Checked-boundary table, scoped no-`uikit` audit for file, focused server tests, full module tests. | Accepted / promoted in `bus-factory` `a65cbc3`; `bus-factory` production audit is clean. |
| Factory browser runtime/action resources | `bus-factory/internal/serve/browser_runtime.go` | Accepted public `pkg/ui` action/resource/effect facade. | Resource method/path/kind preservation, runtime API config, action result semantics. | Symbol/behavior table, focused browser-runtime tests or package tests, full module tests. | Accepted / promoted in `bus-factory` `efb3533`; `bus-factory` production audit is clean. |
| Factory business view primitives | `bus-factory/internal/serve/business_view.go` | Public `pkg/ui` node primitives, CSS/icon helpers, split layout checked helpers. | Generated HTML boundary behavior, split layout classes/styles, action IDs, panel/table markup semantics. | Checked-boundary table, focused render tests if available, scoped no-`uikit` audit, full module tests. | Accepted / promoted in `bus-factory` `c9265f7`; `bus-factory` production audit is clean. |
| Factory AI service DTO/render swaps | `bus-factory/internal/serve/ai.go`, `ai_thread_isolation.go` | Accepted `assistantui` Slice A/B and `terminalui` terminal DTO/public helpers. | AI thread/session DTO shape, model candidates, event filtering, isolation branch/worktree naming, terminal approval/session snapshots. | Facade table, focused AI/server tests, full module tests. | Accepted / promoted in `bus-factory` `b17b33b`; `bus-factory` production audit is clean. |
...
| Chat AI service DTO/panel cleanup | `bus-chat/internal/serve/ai.go`, `ai_workspace_locks.go`, `ai_appserver.go` | Accepted `assistantui` Slice A/B and `terminalui` DTO/helpers where applicable. | `/v1/ai/*` JSON fields, event filtering, model candidates, lock/isolation payloads, terminal approvals/session snapshots, persisted `ai-state.json`. | Symbol/behavior table, scoped no-`uikit` audit over three files, focused serve tests, full module tests. | Accepted / promoted in `bus-chat` `959ded6`; `bus-chat` production audit is clean. Message buffer/history remains adopter scope unless future work proves a missing facade. |
| Ledger server/status helper cleanup | `bus-ledger/internal/server/server.go`, `logging.go` | Accepted `pkg/ui` token/static/client-log/listener/CSS/logger helpers. | Token/static/client-log behavior, logging fields, listener URL and route handling. | Checked-boundary table, scoped no-`uikit` audit for files, focused server tests, full module tests. | Accepted / promoted in `bus-ledger` `643f707`; superseded by final `bus-ledger` production-clean audit. |
| Ledger AI server DTO/panel cleanup | `bus-ledger/internal/server/ai.go`, `ai_runtime_config.go`, `ai_thread_isolation.go` | Accepted `assistantui` Slice A/B and `terminalui` DTO/helpers where applicable. | AI isolation, event/model catalog semantics, terminal approval/session snapshots, runtime config defaults. | Symbol/behavior table, focused server/AI tests, full module tests. | Accepted / promoted in `bus-ledger` `01d5f76`; superseded by final `bus-ledger` production-clean audit. |
| Ledger WASM runtime/app context | `bus-ledger/internal/ui/wasm/app.go`, `app_context.go`, `frontend_errors.go`, `ledger_controller.go` | Accepted `pkg/ui` runtime/mount/error-host, file/dropzone, split/projection facades plus `assistantui` AI panel helpers. | Global access boundaries, render recovery, error forwarding, callback/disposer semantics, AI refresh/drop lifecycle. | Facade/behavior table, native tests, scoped no-`uikit` audit for files, WASM proof or named toolchain exception. | Accepted / promoted as part of `bus-ledger` `769b334`; compact inventory reports 0 `bus-ledger` production hits. |
