...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` control/action/auth bridge still backed by `uikit` conversions | drain `control_uikit_bridge.go` without compatibility wrappers by moving remaining link/icon/event controls, filter toolbar, and credential login card ownership into `pkg/ui`; then rerun deletion probe | active / split required | Hydrated deletion/build-exclusion probe on `bus-ui` `e0c5a45` disabled `pkg/uikit` and first failed at `pkg/ui/control_uikit_bridge.go:5:2`; source map `logs/worker-output/gx-ui-control-bridge-source-map-20260616a.md` split the bridge into link/icon/event controls, filter toolbar, credential login card, and final bridge/probe children. Link/icon/event, filter toolbar, and credential login card are accepted; only final bridge removal/probe remains before adopter refill. Adopter lanes remain parked until this parent is accepted and the next deletion probe names the matrix state. |
...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/runtime_facade.go`, `pkg/ui/runtime_facade_test.go` if needed, focused mounted-app/error/lifecycle files | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` mounted runtime facade still backed by `uikit` | move `MountedApp`, `MountApp`, `NewMountedApp`, `NewMountedNodeApp`, `NewMountedGXApp`, `NewMountedGXNodeApp`, `RenderDOMErrorBannerHTML`, `NormalizeDOMErrorDismissAction`, `OnceDispose`, and `ChainDisposers` into `pkg/ui` using existing local render/runtime/GX helpers, without moving unrelated WASM/browser runtime surfaces | active / next dispatch | First blocker from the post-empty-state hydrated deletion/build-exclusion probe on `bus-ui` `4c8aa07`: `pkg/ui/runtime_facade.go:8:8: no required module provides package github.com/busdk/bus-ui/pkg/uikit`. Source-map signal points to `pkg/uikit/mount_app.go`, `pkg/uikit/mount_gx.go`, `pkg/uikit/error_host.go`, and `pkg/uikit/lifecycle.go`; keep this as the next core implementation lane and rerun the deletion probe immediately after acceptance. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/runtime_facade_js_wasm.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` JS/WASM runtime event facade still backed by `uikit` | source-map `pkg/uikit/wasm_dom_js.go`, `view_mount_js.go`, and callback/listener helpers; move `EventTarget`, `JSEventTarget`, callback retention, event listener, and JS event target helpers into `pkg/ui` with js/wasm tests | queued / known future core | Static production audit after the post-empty-state deletion probe found this future core row. Sequence only after the deletion probe names it or after all earlier core blockers clear; keep adopters parked. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/runtime_gateway_facade.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` gateway runtime facade still backed by `uikit` | source-map runtime API URL and gateway client helpers; move `URLResolver`, gateway client types, `NewHTTPGatewayClient`, and `ResolveAPIURL` into `pkg/ui` with focused native tests | queued / known future core | Static production audit after the post-empty-state deletion probe found this future core row. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/wasm_facade_js_wasm.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` WASM DOM/global facade still backed by `uikit` | source-map JS global access, DOM attribute helpers, click binding, closest element, API URL, and mount-with-scroll preservation helpers; move to `pkg/ui` with js/wasm checks | queued / known future core | Static production audit after the post-empty-state deletion probe found this future core row. |
...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_evidence_facade.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` shell/evidence URL facade still backed by `uikit` | source-map shell asset URL validation/resolution and evidence URL resolver helpers; move to `pkg/ui` with URL-safety and resolver tests | queued / known future core | Static production audit after the post-empty-state deletion probe found this future core row. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_navigation_status.go` | `pkg/uikit` import/calls visible in production static audit | core `pkg/ui` shell/navigation/status facade still backed by `uikit` | split table-first before implementation: shell/page/sidebar/action panel, menu/tabs/navigation, loading/result/error status surfaces, status constants/errors; do not treat as one broad implementation lane unless source map proves coupling | queued / known future core parent | Static production audit after the post-empty-state deletion probe found this future core parent. |
