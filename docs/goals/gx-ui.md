...
- known adopter-discovered public facade gaps: 1 row below (`core-15`), found
  while source-mapping `bus-inspection` after production core imports were
  clean;
...
| core-15 | `pkg/ui/ai_runtime_js.go`, `pkg/ui/ai_callback_registry_js.go`, `pkg/ui/app_scaffold_js.go`, `pkg/ui/runtime_helpers.go`, `pkg/ui/logger.go`, focused js/wasm tests | `pkg/uikit/ai_runtime_js.go`, `pkg/uikit/ai_callback_registry_js.go`, `pkg/uikit/app_scaffold_js.go`, `pkg/uikit/runtime_helpers.go`, `pkg/uikit/logger.go`; already-owned `pkg/ui` dependencies include `ErrorReporter`, `RecoverToReporter`, `HTTPGatewayClient`, `ViewMountOptions`, `MountHTMLWithScrollPreservation`, `Global*`, `SetClassOnSelector`, `ReadDroppedFileBytes`, and `UploadFileMultipart` | public WASM AI runtime/scaffold/callback registry/logging helpers needed by adopter WASM apps; no compatibility shell should remain | planning/implementation needed before `bus-inspection/internal/ui/wasm/app.go` can be a mechanical adopter rewrite. Source-map table from bus-inspection: `uikit.AIRuntime`, `NewAIRuntimeProfile`, `NewAICallbackRegistry`, `WASMAppScaffold`, `NewWASMAppScaffold`, `GoSafeWithReporter`, and `Logger` are still only in `pkg/uikit`; existing public `ui` already owns document/window/location globals, API URL, HTTP gateway client, mount, class toggles, dropped-file read, multipart upload, error reporting, `Dispose`, and view mount options. |
...
| adopter-7 | `bus-inspection` | `internal/ui/wasm/app.go`, `internal/ui/wasm/view.go` | replace wasm app/view composition with public `ui` facades | split required. `app.go` is blocked by adopter-discovered `core-15` runtime/scaffold ownership; public `ui` already has globals/API URL/HTTP gateway/mount/drop upload/error helpers. `view.go` is a separate broad component-composition rewrite and should not be swept together with app runtime. |
