...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_action_panel.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, focused tests | `ShellActionPanel*` aliases still routed through `pkg/uikit` | shell/navigation child: shell action panel ownership | move `ShellActionPanelProps`, `ShellActionPanelControl`, `ShellActionPanelCheckedProps`, `ShellActionPanel`, `ShellActionPanelNodeChecked`, `ShellActionPanelChecked`, `ShellActionPanelHTML`, and shell action panel errors into package-owned `pkg/ui` files using `pkg/uikit/shell_action_panel.go` and `pkg/uikit/shell_action_panel_render.go`; remove only shell-action-panel aliases from `shell_navigation_status.go`; update facade parity in `pkg/ui/ui_test.go` | queued / next implementation | Smallest first child from the source map: exact path preflight passed for `pkg/uikit/shell_action_panel.go`, `pkg/uikit/shell_action_panel_render.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, and `pkg/ui/ui_test.go`. DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped audit proving no `pkg/uikit|uikit\.` in new action-panel files and no `uikit.ShellActionPanel` aliases in `pkg/ui/shell_navigation_status.go`/`pkg/ui/ui.go`. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_page.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, focused tests | `AppShell*`, `PageShell*`, shell constants/errors still routed through `pkg/uikit` | shell/navigation child: app/page shell ownership | move `AppShellProps`, `PageShellProps`, default shell asset constants, shell validation errors, `AppShell`, `AppShellChecked`, `PageShellNodeChecked`, `PageShellChecked`, `PageShell`, and page-shell GX adapter helpers into `pkg/ui` without duplicating already-owned shell asset URL helpers | queued / after action panel unless deletion matrix reorders | Source-map files: `pkg/uikit/shell.go`, `pkg/uikit/page_shell.go`, `pkg/uikit/page_shell_gx_adapter.go`, plus existing `pkg/ui/shell_asset_helpers.go` for ownership boundaries. Keep shell asset URL resolver helpers owned by the already accepted shell-evidence slice; only move app/page shell composition here. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/sidebar_navigation.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, focused tests | `SidebarShell*`, `SidebarNav*`, and sidebar navigation errors still routed through `pkg/uikit` | shell/navigation child: sidebar shell/navigation ownership | move sidebar shell/nav props, checked/node helpers, dispatch, validation errors, and render helpers from `pkg/uikit/sidebar_shell.go` into `pkg/ui`; remove corresponding aliases from `shell_navigation_status.go` and common facade aliases where owned | queued / after source-map child sequencing | Source-map files: `pkg/uikit/sidebar_shell.go` and facade aliases in `pkg/ui/shell_navigation_status.go`. DoD includes sidebar render/dispatch tests and scoped audit for `SidebarShell|SidebarNav|DispatchSidebarNavClick` aliases. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/navigation_primitives.go`, `pkg/ui/shell_navigation_status.go`, `pkg/ui/ui.go`, focused tests | `Menu*`, `Tabs*`, `Navigation*`, navigation events/errors still routed through `pkg/uikit` | shell/navigation child: menu/tabs/navigation ownership | move `MenuItem*`, `MenuProps`, `TabsProps`, `NavigationProps`, click events/handlers, checked/node helpers, dispatch helpers, `ValidateNavigationHref`, `DOMAttrUIValue`, and navigation validation errors from `pkg/uikit/navigation_primitives.go` into `pkg/ui` | queued / after sidebar or as compiler matrix allows | Keep this separate from sidebar because the source map shows a clean menu/tabs/navigation file boundary. DoD includes facade parity in `pkg/ui/ui_test.go`, focused dispatch/href validation tests, and scoped audit for `uikit.Menu|uikit.Tabs|uikit.Navigation|uikit.ValidateNavigationHref`. |
