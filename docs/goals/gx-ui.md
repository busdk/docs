...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/runtime_gateway_facade.go` | `pkg/uikit` import/calls visible in production static audit; first compiler blocker after runtime facade | core `pkg/ui` gateway runtime facade still backed by `uikit` | source-map runtime API URL and gateway client helpers; move `URLResolver`, gateway client types, `NewHTTPGatewayClient`, and `ResolveAPIURL` into `pkg/ui` with focused native tests | active / next dispatch | Hydrated deletion/build-exclusion probe on `bus-ui` `d70b316` disabled `pkg/uikit` and first failed at `pkg/ui/runtime_gateway_facade.go:8:8`; this is the next compiler-derived core blocker. Static production audit confirms this remains one of 9 known core `pkg/ui` rows still carrying `pkg/uikit` references. |
