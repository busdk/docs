...
- known remaining core production rows: 4 implementation/probe rows below,
  across 3 `bus-ui/pkg/ui` production files still importing/calling
  `pkg/uikit` after the accepted WASM runtime-callback slice;
...
the WASM runtime-callback slice reports 3 owner facade production files and 35
...
1. Keep production core first: source-map the four remaining JS/WASM owner
   facades named by static audit (`wasm_facade_js_wasm.go`,
   `ai_drop_facade_js_wasm.go`, and `split_projection_facade_js_wasm.go`)
