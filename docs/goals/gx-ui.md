...
- known adopter-discovered public facade gaps: 1 open; `core-20`, the ledger
  WASM projection/render/controller helper gap, is open after the
  post-`core-19` adopter-9a compile proof. `core-19`, the ledger WASM app
  wiring gap for AI panel events, close guards, action controller, AI href
  resolver, and dropped-path collection, is accepted. `core-15`, the factory
...
- known remaining adopter/user production rows: 5 open rows below, across 2
  modules with 16 refined production hits (`bus-inspection`, `bus-ledger`);
...
- production-clean by refined `uikit` call/import audit but still in the
  ...
  `bus-portal-notes`;
- tests/docs/examples-only rows: 22 refined test files, 30 Markdown files,
...
owner facade production files and 16 remaining production adopter files across
the 5 open adopter rows below after the accepted ledger server runtime/logging
and AI-support slices. Do not treat raw example/test probe failures as
...
| core-20 | parent row for ledger WASM projection/render/controller helper facades | `pkg/uikit` helpers named by post-`core-19` ledger WASM compile feedback: `MountedTextView`, `NewParagraphTextView`, `ParseSplitRouteSelection`, `RenderAIMessageFromLocation`, `BuildAIPanelRenderProps`, `MountAIPreservedView`, `FindSelectedLine`, `NormalizeProjectionListMode`, `ProjectionListModeButtonClasses`, and additional list/detail presenter helpers after the first ten errors | public helpers needed before ledger WASM adopter rows can move to `pkg/ui` types without package-mismatch failures. Do not patch ledger around these with `uikit` imports; source-map the helper family first, then split if independent. | active / `core-20a` accepted, `core-20b` still compile-proof gated. Worker `gx-ui-ledger-wasm-app-runtime-spark-20260616b` false-actived; the reviewed worker-tree rewrite across the coupled ledger WASM files was reverted cleanly after `GOOS=js GOARCH=wasm go test -exec=echo ./internal/ui/wasm` proved these public helpers were still missing. Parent advances only after the next adopter-9a public-selector rewrite proves no remaining public facade gap or names the next concrete child. |
...
| core-20b | likely `pkg/ui/projection_list_panel.go` plus any newly named projection/list/detail helper files | `pkg/uikit/projection_list_panel.go` and any additional source files named by the post-`core-20a` ledger WASM compile proof | projection list/detail presenter helpers such as `NormalizeProjectionListMode`, `ProjectionListHeaders`, `ProjectionListModeButtonClasses`, and possibly rows/title-action VM helpers if adopter-9a still needs them after `core-20a`. | queued / compile-proof gated. Do not implement broad panel VM copying until the next ledger WASM compile names the actual remaining public helpers; if needed, source-map and split by list mode/title actions/rows/detail before implementation. |
...
| adopter-9a | `bus-ledger` | `internal/ui/wasm/app.go`, `app_context.go`, `frontend_errors.go`, `app_context_test.go`, `frontend_errors_test.go` | replace WASM app runtime/scaffold/action/drop/resource/frontend-error helpers with public `pkg/ui` facades | parked behind open `core-20` because app context, ledger controller, split view props, and projection/list/detail helpers share `uikit` concrete types; after `core-20` lands, rerun the public-selector rewrite across the coupled WASM files with scoped audits and JS/WASM compile proof |
| adopter-9b | `bus-ledger` | `internal/ui/wasm/ledger_controller.go`, `ledger_controller_test.go` | replace controller projection query, route, mount, render-runtime, AI panel render-prop, and DTO helpers with public facades | split from adopter-9 because it touches projection/render/split composition and overlaps adopter-10/11 behavior; source-map before implementation |
