...
- known remaining core production rows: 1 final audit/probe row below, with 0
...
facade production files and 32 production adopter/core files. Do not treat raw
example/test probe failures as adopter-ready production work.
...
| core-14 | `pkg/ui` owner audit and `pkg/uikit`/`pkg/uikit/uikittest` deletion probe | all accepted replacement files | no production `pkg/ui` backing import/call to `pkg/uikit`; old package is not retained as compatibility layer | active final core truth gate: refined owner-facade production audit is clean after `bus-ui` `cc4a0c4`; remaining probe failures must be split into test/example/adopter rows before adopter implementation resumes |
