...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` control/action/auth bridge still backed by `uikit` conversions | drain `control_uikit_bridge.go` without compatibility wrappers by moving remaining link/icon/event controls, filter toolbar, and credential login card ownership into `pkg/ui`; then rerun deletion probe | active / split required | Hydrated deletion/build-exclusion probe on `bus-ui` `e0c5a45` disabled `pkg/uikit` and first failed at `pkg/ui/control_uikit_bridge.go:5:2`; source map `logs/worker-output/gx-ui-control-bridge-source-map-20260616a.md` splits the bridge into link/icon/event controls, filter toolbar, credential login card, and final bridge/probe children. Adopter lanes remain parked until this parent is accepted and the next deletion probe names the matrix state. |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go`, likely `pkg/ui/action_controls.go`, focused tests | `LinkButton`, `IconButton`, and `EventBar` helpers still call through `pkg/uikit` | control bridge child: link/icon/event controls | move `LinkButtonProps`, `LinkButton`, `LinkButtonChecked`, `LinkButtonNodeChecked`, `IconButtonProps`, `IconButton`, `IconButtonChecked`, `IconButtonNodeChecked`, `EventBarAction`, `EventBarProps`, `EventBar`, `EventBarChecked`, and `EventBarNodeChecked` into `pkg/ui` using local `ControlProps`, `ButtonProps`, `ButtonVariant`, `ButtonSize`, and `IconProps` | queued | DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, focused render/validation tests adapted from `pkg/uikit/control_primitives*_test.go`, and scoped audit proving no link/icon/event `uikit.` calls remain in `pkg/ui/control_uikit_bridge.go` |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go`, likely `pkg/ui/filter_toolbar.go`, focused tests | `FilterToolbar` helpers still call through `pkg/uikit` | control bridge child: filter toolbar | move `FilterToolbarResetHandler`, `FilterToolbarResetEvent`, `FilterToolbarProps`, filter toolbar errors, checked/node helpers, and submit/reset dispatch helpers into `pkg/ui` using local form/field/button helpers | queued | DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, focused toolbar render/dispatch/validation tests, and bridge audit proving filter-toolbar conversions and `uikit.FilterToolbar*` calls are gone |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go`, likely `pkg/ui/credential_login_card.go`, focused tests | `CredentialLoginCard` helpers still call through `pkg/uikit` | control bridge child: credential login card | move `CredentialLoginCardProps`, credential login checked/node helpers, and required credential card render/config helpers into `pkg/ui` using local form/field/input/button helpers | queued | DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, focused credential card render/dispatch/validation tests, and bridge audit proving credential card conversions and `uikit.CredentialLoginCard*` calls are gone |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/control_uikit_bridge.go`, `pkg/ui/ui.go` if needed | final control bridge `pkg/uikit` import/calls | control bridge child: final bridge removal and truth gate | delete `control_uikit_bridge.go` if empty or reduce it only to fresh compiler-derived non-bridge work, then rerun hydrated deletion/build-exclusion probe | queued | DoD: `go test ./pkg/ui`, `go test ./...`, `git diff --check`, scoped audit for `pkg/uikit|uikit\.` in `pkg/ui/control_uikit_bridge.go`, and deletion probe advances beyond `pkg/ui/control_uikit_bridge.go` |
