...
| `bus-ui` | `bus-portal-notes` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | AI DTO/helpers/panel render/client-script implementation moved into `pkg/assistantui`; node-first `RenderAIPanel` plus explicit `RenderAIPanelHTML` boundary | implementation accepted / truth gate pending | implementation accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`); deletion-probe rerun must prove matrix advances |
| `bus-ui` | `bus-portal-ai` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | same assistantui rewrite | implementation accepted / truth gate pending | implementation accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`); deletion-probe rerun must prove matrix advances |
...
./pkg/assistantui`, and `go test ./...` in `bus-ui`. This remains
`implementation accepted / truth gate pending` for deletion-probe velocity
accounting until the hydrated deletion/build-exclusion probe is rerun on a
BusDK base pinned to `bus-ui` `17dddd5` and this matrix is updated with the
next compiler blocker or a clean-through-owner proof.
