...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_navigation_status.go` | `pkg/uikit` import/calls visible in production static audit; first compiler blocker after shell evidence | core `pkg/ui` shell/navigation/status facade still backed by `uikit` | split table-first before implementation: shell/page/sidebar/action panel, menu/tabs/navigation, loading/result/error status surfaces, status constants/errors; do not treat as one broad implementation lane unless source map proves coupling | active / source-map next | Hydrated deletion/build-exclusion probe after shell evidence now stops at `pkg/ui/shell_navigation_status.go:8:8`. Static production audit also shows this broad parent, so the next dispatch should be a path-preflighted source-map/table split before implementation; keep adopters parked until this parent advances. |
