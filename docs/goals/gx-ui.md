...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/form_controls_input.go`, `pkg/ui/ui.go` | `InputProps`, `TextAreaProps`, `SelectProps` still aliased from `pkg/uikit` | form-controls child: input/select/text area ownership | move `InputType`, `InputProps`, `TextAreaProps`, `SelectOption`, `SelectProps`, input errors, and checked/node helpers into `pkg/ui` | active / parked no-diff worker | Mini worker `gx-ui-form-controls-input-select-mini-20260616a` (`task-3331264155fc`) passed module materialization on `bus-ui` `459f252` and produced source-read evidence, but stayed clean after the exact patch-target nudge and was stopped as execution-path failure; next attempt should use the reviewed worker-owned exception path or a tighter prebuilt patch table. DoD: input/text area/select render and failure-path tests, generic file-input rejection preserved, `go test ./pkg/ui`, `go test ./...`, scoped no-`uikit` audit for input files and aliases; review must state whether `pkg/ui/control_uikit_bridge.go` shrinks, remains, or is no longer needed for each affected boundary |
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/form_controls_form.go`, `pkg/ui/ui.go` | `FormProps`, `FieldProps`, form/field helpers still aliased from `pkg/uikit` | form-controls child: form/field ownership | move `FormMethod`, `FormProps`, `FieldControlRenderer`, `FieldProps`, `ValidateFormMethod`, `ValidateFormAction`, `FormNodeChecked`, `FieldNodeChecked`, and `appendDescribedBy` into `pkg/ui` | queued | DoD: form method/action, source, and field described-by tests; `go test ./pkg/ui`; scoped no-`uikit` audit for form/field files and aliases |
