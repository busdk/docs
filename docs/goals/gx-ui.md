...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/form_controls_file_input.go`, `pkg/ui/ui.go` | `FileInputProps`, file-input render helpers, and accepted-types attr helper still aliased from `pkg/uikit` | form-controls child: file input ownership | move `FileInputProps`, file-input accessible-name/render behavior, `DropAcceptedTypesAttr`, `FileInputChecked`, and `FileInputNodeChecked` into `pkg/ui` | queued | Split out before implementation because file input can land independently from dropzone DTO/policy behavior. DoD: file input accessibility/accepted-types render tests, unsafe attr/missing accessible-name/invalid callback tests, facade node test, `go test ./pkg/ui`, `go test ./...`, scoped no-`uikit` audit for file-input files and aliases |
