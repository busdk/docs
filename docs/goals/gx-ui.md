...
| `bus-ui` | `bus-portal-notes` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | AI DTO/helpers/panel render/client-script implementation moved into `pkg/assistantui`; node-first `RenderAIPanel` plus explicit `RenderAIPanelHTML` boundary | accepted | accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`) |
| `bus-ui` | `bus-portal-ai` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | same assistantui rewrite | accepted | accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`) |
| `bus-ui` | `bus-portal-accounting` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | same assistantui rewrite | accepted | accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`) |
| `bus-ui` | `bus-portal-auth` | `bus-ui/pkg/assistantui/assistantui_ai_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `assistantui` facade still backed by `uikit` | same assistantui rewrite | accepted | accepted in `bus-ui` `17dddd5` (`task-6c8988b9e1cc`) |
...
Assistantui core reroute: `bus-ui` `17dddd5` (`task-6c8988b9e1cc`, worker
...
./pkg/assistantui`, and `go test ./...` in `bus-ui`. Next required truth gate:
rerun the hydrated deletion/build-exclusion probe on a BusDK base pinned to
`bus-ui` `17dddd5` and update this matrix with the next compiler blocker.
