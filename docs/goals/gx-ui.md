...
| Core ui action/resource facade implementation | `bus-ui/pkg/ui/action_resource_facade.go`, plus focused action/resource tests and any new `pkg/ui` or non-compatibility internal implementation files needed by the source map | Move action/resource state/result DTOs, resource clients, browser/gateway fetch options, multipart payloads, and action/resource helper constructors out of `pkg/uikit`; do not keep alias wrappers to the deleted package. | Action/result status and field error shape, resource kind/client interfaces, browser fetch request/response semantics, multipart payload fields, helper validation and result-kind behavior. | Source-map table first; `go test ./pkg/ui`; `go test ./...` in `bus-ui`; scoped no-production `pkg/uikit` audit for touched `pkg/ui` files; follow-up deletion-probe rerun advances beyond `pkg/ui/action_resource_facade.go`. | Active, next core blocker from deletion rerun `task-a1f0c7192d7f`. |
