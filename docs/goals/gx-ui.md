...
- known adopter-discovered public facade gaps: 0 open; `core-19`, the ledger
  WASM app wiring gap for AI panel events, close guards, action controller, AI
  href resolver, and dropped-path collection, is accepted. `core-15`, the
...
| adopter-9a | `bus-ledger` | `internal/ui/wasm/app.go`, `app_context.go`, `frontend_errors.go`, `app_context_test.go`, `frontend_errors_test.go` | replace WASM app runtime/scaffold/action/drop/resource/frontend-error helpers with public `pkg/ui` facades | implementation-ready after accepted `core-19`; rerun with selector table: browser globals, API URL/prompt/confirm, error reporter/recover, JS event targets, disposers, WASM app scaffold, AI runtime/callback registry/action/drop controllers, AI refresh host/callbacks, split layout state, projection query client, evidence/AI href resolvers, split resize wiring, window lifecycle, frontend error forwarding, and console logging -> `ui.*`; DoD is focused WASM package tests, `GOOS=js GOARCH=wasm go test -exec=echo ./internal/ui/wasm`, full module tests where applicable, scoped no-`uikit` audit for target files, compact inventory |
