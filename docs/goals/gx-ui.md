...
| core-20 | `pkg/ui` projection/render/controller helper facades, exact target files TBD by source-map table | `pkg/uikit` helpers named by post-`core-19` ledger WASM compile feedback: `MountedTextView`, `NewParagraphTextView`, `ParseSplitRouteSelection`, `RenderAIMessageFromLocation`, `BuildAIPanelRenderProps`, `MountAIPreservedView`, `FindSelectedLine`, `NormalizeProjectionListMode`, `ProjectionListModeButtonClasses`, and additional list/detail presenter helpers after the first ten errors | public projection/render/controller helpers needed before ledger WASM adopter rows can move to `pkg/ui` types without package-mismatch failures. Do not patch ledger around these with `uikit` imports; source-map the full helper family first, then split if independent. | open / planning table required. Worker `gx-ui-ledger-wasm-app-runtime-spark-20260616b` false-actived; the reviewed worker-tree rewrite across the coupled ledger WASM files was reverted cleanly after `GOOS=js GOARCH=wasm go test -exec=echo ./internal/ui/wasm` proved these public `pkg/ui` helpers were still missing. Next action: table-first source map for core-20, then dispatch the smallest implementation-ready public facade slice before rerunning adopter-9a. |
