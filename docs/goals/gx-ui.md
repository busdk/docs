...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/portal_launcher_empty_state.go`, likely `pkg/ui/shell_navigation_status.go` and `pkg/ui/ui.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` portal launcher empty-state facade still backed by `uikit` status surface aliases | move portal launcher empty-state rendering onto package-owned `pkg/ui` empty-state/status surface helpers, or prove a smaller owner-file-only move is sufficient with the deletion probe | active / source-map required | New first compiler-derived blocker from the post-control-bridge hydrated deletion/build-exclusion probe on `bus-ui` `4089eae`: `pkg/ui/portal_launcher_empty_state.go:12:2: no required module provides package github.com/busdk/bus-ui/pkg/uikit`. Do not unpark adopters until this row lands and the next deletion probe advances. |
