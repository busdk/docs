...
- known remaining core production rows: 2 implementation/probe rows below,
  across 1 `bus-ui/pkg/ui` production file still importing/calling
  `pkg/uikit` after the accepted AI-drop WASM slice;
...
- production-clean by refined `uikit` call/import audit but still in the
  ...
  `bus-portal-notes`;
- tests/docs/examples-only rows: 27 refined test files, 30 Markdown files,
...
Current deletion sequencing probe result: with the accepted AI-drop patch copied
to `/private/tmp/bus-ui-uikit-deletion-probe-ai-drop` and `pkg/uikit` moved
outside the module, raw `go test ./...` still stops first in examples/test
harness setup (`examples/completedapis/render_test.go` imports
`github.com/busdk/bus-ui/pkg/uikit/uikittest`). The owner-package probe now
advances beyond AI drop: native `go test ./pkg/ui` stops at the test-only
`pkg/ui/split_projection_facade_test.go` `pkg/uikit` import, and
`GOOS=js GOARCH=wasm go test -exec=echo ./pkg/ui` stops at the remaining
production core blocker `pkg/ui/split_projection_facade_js_wasm.go:5:8`.
The compact static production audit after the AI-drop slice reports 1 owner
facade production file and 33 production adopter/core files. Do not treat raw
...
|---|---|---|---|
| test-1 | 27 refined `*_test.go` files outside `pkg/uikit` and examples | test-only parity/harness residue | update after corresponding production rows land; do not count as production backlog |
...
1. Keep production core first: source-map and implement the remaining JS/WASM
   owner facade named by static audit (`split_projection_facade_js_wasm.go`)
   before adopter implementation.
