...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/action_resource_facade.go` | `github.com/busdk/bus-ui/pkg/uikit` | core `pkg/ui` action/resource facade still backed by `uikit` | move shared action/resource DTOs, clients, resource fetch/multipart helpers, and checked action/resource helpers into `pkg/ui` or a non-compatibility internal implementation owned by `pkg/ui` | active | next compiler blocker from deletion rerun `task-a1f0c7192d7f`; source-map table required before implementation; likely followed by `pkg/ui` WASM/form facade and test-harness replacement rows |
...
| Core ui action/resource facade implementation | `bus-ui/pkg/ui/action_resource_facade.go`, plus focused action/resource tests and any new `pkg/ui` or non-compatibility internal implementation files needed by the source map | Move action/resource state/result DTOs, clients, and helper constructors out of `pkg/uikit`; browser/fetch/multipart transport should be a separate follow-up if budget is tight. Do not keep alias wrappers to the deleted package. | Action/result status and field error shape, resource kind/client interfaces, helper validation and result-kind behavior; browser fetch request/response semantics and multipart payload fields are follow-up invariants unless included deliberately. | Source-map table accepted; next implementation should start with non-browser action/result/resource primitives only, then `go test ./pkg/ui`; scoped no-production `pkg/uikit` audit for touched `pkg/ui` files; follow-up deletion-probe rerun advances beyond `pkg/ui/action_resource_facade.go`. | Active, budget-critical handoff. Current compiler blocker is `pkg/ui/action_resource_facade.go:8:8`. Combined implementation task `task-a09c65b8f6af` produced a table but no diff; parked workers: `gx-ui-core-ui-action-resource-mini-20260615a` and `gx-ui-core-ui-action-resource-spark-20260615b`. Smaller task `task-f2e3d82db9aa`, worker `gx-ui-core-ui-action-resource-core-mini-20260615c`, produced only a temporary `PLAN.md` proof diff and was stopped. Smallest next task: table is done; implement only `pkg/ui/action_resource_effect.go` non-browser core, remove moved aliases from `action_resource_facade.go`/`ui.go`, add focused `pkg/ui` tests. |
