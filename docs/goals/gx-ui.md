...
| `bus-ui` | `bus-ui` | `bus-ui/pkg/ui/shell_evidence_facade.go` | `pkg/uikit` import/calls visible in production static audit; first compiler blocker after runtime gateway | core `pkg/ui` shell/evidence URL facade still backed by `uikit` | source-map shell asset URL validation/resolution and evidence URL resolver helpers; move to `pkg/ui` with URL-safety and resolver tests | active / next dispatch | Hydrated deletion/build-exclusion probe after runtime gateway now stops at `pkg/ui/shell_evidence_facade.go:14:2`. Static production audit also shows this row, so this is the next compiler-derived core lane; keep adopters parked until this row lands, promotes, and its deletion probe advances. |
