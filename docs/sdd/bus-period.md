...
- `--equity-account <code>`: (Optional.) Account code for the balancing equity line (e.g. retained earnings). Default: implementation-defined; the SDD mandates that the default be documented in the module (e.g. a common code such as 3200 or a value derived from workspace config if available). Rationale for default: many entities use a single retained-earnings or carry-forward account so a sensible default reduces required flags in the common case.
