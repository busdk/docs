...
Interface IF-JRN-001 (module CLI). The module exposes `bus journal` with subcommands `init`, `add`, `balance`, `classify`, and `template` and follows BusDK CLI conventions for deterministic output and diagnostics.
...
Documented parameters for `bus journal add` are `--date <YYYY-MM-DD>`, `--desc <text>`, `--debit <account>=<amount>`, and `--credit <account>=<amount>`. The add command accepts optional source-key idempotency (`--source-id <key>`, optional `--source-system <name>`, `--if-missing`) and deterministic bulk replay input (`--bulk-in <file|->`, JSON array or NDJSON). Bulk mode applies the same validation and idempotency semantics per transaction and emits deterministic summary diagnostics. Documented parameters for `bus journal balance` include `--as-of <YYYY-MM-DD>`.
...
**Idempotent posting with source keys and bulk replay input.** Implemented extension: `bus journal add` supports `--source-id <key>`, optional `--source-system <name>`, and `--if-missing` for deterministic skip-if-exists semantics (uniqueness on `(source_system, source_id)` per period). Replay scripts can run re-entrantly without marker hacks. Bulk replay input is implemented via `--bulk-in <file|->` (JSON array or NDJSON) and applies the same validation and idempotency checks transaction-by-transaction with deterministic summary diagnostics.

**Rule-based bank classification and posting.** Implemented extension: `classify bank` produces deterministic proposal rows from bank transactions using predicates (counterparty regex, message code/text, sign, amount constraints, reference patterns), and `classify apply` posts approved rows as balanced entries with idempotent source-linking (`bank:<bank_txn_id>`). Proposal rows carry confidence and skip-reason traces.

**Learning classifications from prior-year bookkeeping.** Implemented extension: `classify learn` suggests account mappings from historical posted data (e.g. infer frequent `(counterparty/message-pattern/sign) -> (debit_account, credit_account)` mappings from prior periods, emit ranked candidate rules, export YAML profile candidates for classifier use).

**Posting templates with automatic VAT split.** Implemented extension: `template post` and `template apply` support template-driven VAT split (base + VAT) with deterministic rounding, trace fields, and idempotent bank-row linking.

**Loan-payment classifier with principal/interest split.** Implemented extension: `classify bank --loan-profiles` (and aliases `loan-propose` / `loan-apply`) supports lender-profile matching and deterministic principal/interest(/fee) proposal splits, including schedule-aware fallback diagnostics.

**Suspense fallback and reclassification workflow.** Implemented extension: unmatched classify rows can be routed to a configured suspense account via `classify bank --suspense-account --bank-account --suspense-reason`, and later resolved with `classify suspense-propose` / `classify suspense-apply` using deterministic selectors, reason metadata, dry-run, and idempotent reclassification voucher ids.
