...
Command `bus data table assert <table>` takes a required table path, validates it with the beside-the-table schema, applies optional repeatable equality filters, and evaluates deterministic row-count thresholds (`--min-rows`, `--max-rows`, `--eq-rows`). At least one threshold is required. Output is machine-readable TSV or JSON with `table_path`, `observed_rows`, configured thresholds, `filters`, and `status`. The command exits `0` on pass and `1` on threshold failure.
...
Commands:
  ...
  table assert <table_path>           Validate table and assert deterministic row-count thresholds.
