...
Bus Config owns workspace-level configuration stored in `datapackage.json` at the workspace root. The workspace descriptor holds the BusDK extension metadata (the `busdk.accounting_entity` object) that defines accounting entity settings — company name, business ID (e.g. Finnish Y-tunnus), legal form, base currency, fiscal year boundaries, VAT registration, VAT reporting cadence, VAT timing basis, VAT command defaults (`vat_default_source` and `vat_default_basis`), optional VAT registration dates for partial first or last periods, and Finnish statutory reporting profile keys used by statement outputs. Other modules read these settings when they validate, post, reconcile, report, or produce filings.
...
FR-CFG-001 Workspace configuration creation. The module MUST provide an `init` command that creates or ensures `datapackage.json` at the workspace root with a valid `busdk.accounting_entity` object. When the file already exists and already contains `busdk.accounting_entity`, `init` MUST print a warning to standard error and exit with code 0 without modifying the file. When the file is missing or does not contain `busdk.accounting_entity`, `init` MUST create or update it so that the descriptor conforms to the [workspace configuration](../data/workspace-configuration) shape. The `init` command MUST accept the same optional accounting-entity flags as `set` (batch form), including base settings (currency, fiscal year, VAT) and Finnish statutory reporting profile settings (`reporting_profile.fi_statutory`). When init creates or adds the `busdk.accounting_entity` subtree, any provided flag overrides the default for that property; when the file already contains `busdk.accounting_entity`, init does not write and the flags are ignored. Acceptance criteria: running `bus config init` in an empty directory creates `datapackage.json` with default accounting entity settings; running it again yields a warning and no write; running init with explicit flags creates the descriptor with those values; running init in a directory that has a minimal data package without the BusDK extension adds the subtree, with any provided flags applied.

FR-CFG-002 Accounting entity settings update. The module MUST support updating accounting entity properties in an existing workspace `datapackage.json` without re-running init. The module MUST provide (1) a batch form `bus config set ...` where only provided flags alter the stored value, and (2) per-property forms `bus config set <key> <value>`. The supported settings include both base accounting entity properties (company name, business ID, legal form, currency, fiscal year, VAT settings and VAT command defaults) and Finnish statutory reporting profile properties (`reporting-standard`, `report-language`, `income-statement-scheme`, `comparatives`, `presentation-currency`, `presentation-unit`, `prepared-under-pma`, signature metadata). Acceptance criteria: both forms update only the specified properties; the command fails with a clear error if `datapackage.json` does not exist or does not contain `busdk.accounting_entity`; unknown key in the per-property form yields exit 2.
...
NFR-CFG-002 Path exposure via Go library. The module MUST expose a Go library API that returns the workspace-relative path to the workspace configuration file (`datapackage.json`). Other modules that need read-only access to the workspace descriptor (e.g. to read accounting entity settings) MUST obtain the path from this module’s library when they need the file path, not by hardcoding `datapackage.json`. The API MUST be designed so that future dynamic path configuration can be supported without breaking consumers. Acceptance criteria: the library provides a path accessor for the workspace config file; consumers use it for read-only path resolution; no consumer hardcodes the config file name outside this module.
...
KD-CFG-004 Path exposure for read-only consumption. The module exposes a path accessor in its Go library for the workspace configuration file so that other modules can resolve its location for read-only access. Write access and all configuration business logic remain in this module.

### Component Design and Interfaces

Interface IF-CFG-001 (init). The module is invoked as `bus config init ...` with optional flags for accounting entity base settings and Finnish statutory reporting profile settings. It follows BusDK CLI conventions for deterministic output and diagnostics. The command accepts no positional arguments. It creates or updates `datapackage.json` at the effective workspace root (current directory or the directory given by `-C` / `--chdir`). When the file is missing, init MUST call the bus-data library to create the empty descriptor (profile, empty resources) first; then init adds or updates the `busdk.accounting_entity` subtree. When the file exists but does not contain `busdk.accounting_entity`, init adds the subtree; any provided accounting-entity flag sets that property (others use defaults). When the file already contains `busdk.accounting_entity`, it exits 0 and prints a warning to stderr without writing; flags are ignored. Flag values must conform to the same schema as for `set`; invalid values produce exit code 2.

Interface IF-CFG-002 (set, batch). The module is invoked as `bus config set ...` with optional flags for accounting entity base settings and Finnish statutory reporting profile settings. The command requires a workspace that already contains `datapackage.json` with a `busdk.accounting_entity` object. Only flags explicitly provided are written; other properties remain unchanged. Flag values must conform to the canonical schema; invalid values produce a deterministic usage error and exit code 2. When no property flags are given, the command exits 0 without modifying the file (no-op).

Interface IF-CFG-003 (set, per-property). The module is invoked as `bus config set <key> <value>` where `<key>` includes both base accounting-entity keys and Finnish statutory reporting profile keys documented in [Workspace configuration (`datapackage.json` extension)](../data/workspace-configuration). The command requires the same workspace preconditions as IF-CFG-002. Only the specified property is updated. Unknown `<key>` produces exit code 2 with a deterministic usage error. Value validation is the same as for the batch form.

Interface IF-CFG-004 (path accessor, Go library). The module exposes a Go library function that returns the workspace-relative path to the workspace configuration file (e.g. `datapackage.json`). Given a workspace root path, the library returns the config file path; resolution MUST allow future override from environment or convention. Other modules use this accessor for read-only path resolution; all writes and config logic remain in this module.
...
**VAT command defaults.** `vat_default_source` (CLI `--vat-default-source`, key `vat-default-source`) and `vat_default_basis` (CLI `--vat-default-basis`, key `vat-default-basis`) define workspace defaults for VAT command execution when `--source` and `--basis` are omitted. Allowed source values are `invoice`, `journal`, and `reconcile`. Allowed basis values are `accrual` and `cash`. `cash` requires source `journal` or `reconcile`.
...
Other modules that need the path to the workspace configuration file for read-only access (e.g. to load accounting entity settings) MUST obtain it from this module’s Go library (IF-CFG-004). All writes and configuration logic remain in this module.
...
**Accounting entity settings:** the set of properties (base currency, fiscal year, VAT registration, VAT reporting period, VAT timing, optional VAT registration start/end, and statutory reporting profile settings) that apply to the whole workspace and are read by other modules.
