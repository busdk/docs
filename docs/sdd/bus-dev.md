...
- **CLI layer.** Parses global flags (including BusDK-standard `-C`, `-o`, `-v`, `-q`, `--help`, `--version`) and the `bus dev` subcommand, resolves the effective working directory, and resolves the agent runtime selection: per-command flag overrides session-stored preference; when neither is set, the implementation detects enabled agents (CLI command exists in PATH) and picks one automatically in a documented deterministic order (FR-DEV-005b, FR-DEV-005d). It then delegates to the appropriate subcommand handler.
