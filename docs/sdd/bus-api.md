...
The built-in module adapters in the standalone `bus-api` binary are implemented in `bus-api` and use Go library calls (`bus-data`) for module-scoped discovery, schema reads, row read/write endpoints, and validation endpoints under `/{token}/v1/modules/{module}/resources/...` and `/{token}/v1/modules/{module}/validate`. Modules also expose semantic entity endpoints under `/{token}/v1/modules/{module}/entities`, `/{token}/v1/modules/{module}/entities/{alias}`, and `/{token}/v1/modules/{module}/entities/{alias}/summary` to make alias-to-resource mapping and high-level summary data explicit for clients. Workflow endpoints are exposed at `/{token}/v1/modules/{module}/workflows` and `/{token}/v1/modules/{module}/workflows/{name}/run` for shared module-level operations (`module-overview`, `validate-module`, and alias-scoped `entity-healthcheck`) and module-specific operations (`accounts-catalog-overview`, `accounts-structure-audit`, `journal-entries-overview`, `journal-batch-health`, `vat-readiness`, `vat-period-rollup`). Richer workflow outputs include journal imbalance signals (`hasDebitColumn`, `hasCreditColumn`, `rowsWithAmounts`, `imbalancedRows`, `balancedRows`) and VAT period rollup signals (`periodField`, `knownPeriods`, `knownPeriodRows`, `unknownPeriodRows`). Strict workflow contracts return deterministic `422` responses with `code=missing_business_fields` when required business fields are missing (journal: `debit`/`credit`; VAT rollup: period-like field). OpenAPI for workflow runs uses typed `oneOf` response schemas per workflow (for example `WorkflowJournalBatchHealthResponse`, `WorkflowVATPeriodRollupResponse`) on top of shared request/error schemas, with canonical schema-level `example` payloads for request/response integration guidance; discriminator mapping on `workflow` is used for response typing; the `{name}` path parameter is enum-constrained per module to currently exposed workflows. Module profiles may additionally define module-specific alias endpoints (for example `/{token}/v1/modules/accounts/accounts/...`, `/{token}/v1/modules/journal/entries/...`, `/{token}/v1/modules/data/items/...`) and alias summary endpoints (`.../{alias}/summary`) that map to module resources while keeping the same library-only integration model. Adapter mutating endpoints honor operation gating (`--read-only`) and return 403 when writes are disabled.
