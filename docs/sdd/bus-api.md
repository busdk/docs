...
* `RouteMappings` and provider capability map (optional; deterministic HTTP method+path to event request/reply mappings validated against declared capabilities)
...
* `--enable-module <name>` repeatable; enables the module with the given identifier (e.g. `accounts` for bus-accounts). `all` enables all built-in module adapters. Only enabled modules are mounted; when no `--enable-module` is given, no module endpoints are exposed.
...
* `GET /{token}/v1/diagnostics` → runtime counters for route dispatch diagnostics (unmapped routes, unknown response events, dispatch timeouts, invalid provider responses, publish/subscription failures).
...
The built-in module adapters in the standalone `bus-api` binary are implemented in `bus-api` and use Go library calls (`bus-data`) for module-scoped discovery, schema reads, row read/write endpoints, and validation endpoints under `/{token}/v1/modules/{module}/resources/...` and `/{token}/v1/modules/{module}/validate`. Resource ownership is explicit and library-based for all built-ins; there is no generic name-prefix ownership fallback. Data-owning modules resolve ownership through their module path libraries (for example `bus-accounts`, `bus-period`, `bus-balances`, `bus-journal`, `bus-bank`, `bus-invoices`, `bus-vat`, `bus-entities`, `bus-inventory`, `bus-loans`, `bus-attachments`, `bus-assets`, `bus-budget`, `bus-payroll`, `bus-reconcile`, and `bus-reports`). Built-ins without owned datasets return empty module resource sets. Module root/resources/validate payloads are runtime contract-validated and modeled in OpenAPI via shared schemas (`ModuleInfoResponse`, `ModuleValidateResponse`, `ModuleValidateError`). Module resource schema/rows/row/validate payloads are also runtime contract-validated and modeled in OpenAPI via shared schemas (`ModuleResourceSchemaResponse`, `ModuleResourceRowsResponse`, `ModuleResourceRowResponse`, `ModuleResourceValidateResponse`); module endpoint error schemas compose a shared base `ModuleEndpointError` (`code`, `message`); failures on these resource endpoints use a stable resource-scoped error contract via `ModuleResourceError` (`code`, `message`, `resource`). Modules also expose semantic entity endpoints under `/{token}/v1/modules/{module}/entities`, `/{token}/v1/modules/{module}/entities/{alias}`, and `/{token}/v1/modules/{module}/entities/{alias}/summary` to make alias-to-resource mapping and high-level summary data explicit for clients; these entity responses are runtime contract-validated and modeled in OpenAPI via shared schemas (`ModuleEntityItem`, `ModuleEntityListResponse`, `ModuleEntitySummaryResponse`), and entity endpoint failures use a stable entity-scoped error contract via `ModuleEntityError` (`code`, `message`, `entity`, `resource`). Workflow endpoints are exposed at `/{token}/v1/modules/{module}/workflows`, `/{token}/v1/modules/{module}/workflows/{name}`, and `/{token}/v1/modules/{module}/workflows/{name}/run` for shared module-level operations (`module-overview`, `validate-module`, and alias-scoped `entity-healthcheck`) and module-specific operations (`accounts-catalog-overview`, `accounts-structure-audit`, `journal-entries-overview`, `journal-batch-health`, `vat-readiness`, `vat-period-rollup`). Workflow list entries include stable `workflowId` values in addition to workflow `name`, `description`, and `category`, and detail lookup by `{name}` returns normalized workflow metadata for either workflow name or workflow ID token. Richer workflow outputs include journal imbalance signals (`hasDebitColumn`, `hasCreditColumn`, `rowsWithAmounts`, `imbalancedRows`, `balancedRows`) and VAT period rollup signals (`periodField`, `knownPeriods`, `knownPeriodRows`, `unknownPeriodRows`). Strict workflow contracts return deterministic `422` responses with `code=missing_business_fields` when required business fields are missing (journal: `debit`/`credit`; VAT rollup: period-like field). OpenAPI for workflow runs uses typed `oneOf` response schemas per workflow (for example `WorkflowJournalBatchHealthResponse`, `WorkflowVATPeriodRollupResponse`) on top of shared request/error schemas, with canonical schema-level `example` payloads for request/response integration guidance; discriminator mapping on `workflow` is used for response typing; the `{name}` path parameter is enum-constrained per module to currently exposed workflow names and workflow ID tokens. Runtime workflow execution accepts either token and normalizes responses to canonical workflow `name` plus validated `workflowId`; workflow endpoint failures use a stable workflow-scoped error contract via `ModuleWorkflowError` (`code`, `message`, `workflow`). Module profiles may additionally define module-specific alias endpoints (for example `/{token}/v1/modules/accounts/accounts/...`, `/{token}/v1/modules/journal/entries/...`, `/{token}/v1/modules/data/items/...`) and alias summary endpoints (`.../{alias}/summary`) that map to module resources while keeping the same library-only integration model; alias endpoint failures use a stable alias-scoped error contract via `ModuleAliasError` (`code`, `message`, `alias`, `resource`) and alias validate success is modeled by `ModuleAliasValidateResponse`. Adapter mutating endpoints honor operation gating (`--read-only`) and return 403 when writes are disabled.
