...
FR-VAT-006 Reconcile cash coverage diagnostics and strict gating. The module MUST emit deterministic coverage diagnostics for `--source reconcile --basis cash` and enforce strict coverage failure by default when coverage is below configured thresholds. Acceptance criteria: report/export/review include machine-readable coverage metrics and source refs; partial coverage exits non-zero by default; explicit opt-out via `--force-partial-coverage` allows continuation with warning; strict mode exits non-zero when thresholds are not met, including `matched_sales_share`, `matched_purchase_share`, and `unmatched_cash_rows` policy thresholds (`--min-sales-coverage`, `--min-purchase-coverage`, `--max-unmatched-cash-rows`).
...
Bus VAT depends on invoice and journal datasets and on VAT reference datasets such as `vat-rates.csv`. It reads accounting entity settings from the workspace `datapackage.json` (maintained by [bus-config](../sdd/bus-config)): `vat_registered`, `vat_reporting_period` (current/default period length), `vat_timing`, `vat_default_source`, `vat_default_basis`, and optionally `vat_registration_start` and `vat_registration_end`. The VAT module owns the authoritative list of period boundaries; it uses those workspace settings as inputs and may maintain a period-definition dataset (e.g. `vat-periods.csv`) or logic to produce periods that can include transitions and non-standard lengths. The VAT engine uses that period list for allocation and reporting, `vat_timing` to choose the allocation date (performance, invoice, or payment date), and VAT command defaults when `--source`/`--basis` are omitted. Missing datasets or schemas result in deterministic diagnostics.
...
Reconcile-evidence cash mode is implemented on `bus vat report|export|review --source reconcile --basis cash`, including deterministic coverage diagnostics (`COVERAGE` output line with matched sales share, matched purchase share, unmatched cash rows, and source refs). Strict coverage gating is the default behavior in this mode, thresholds are controlled by `--min-sales-coverage`, `--min-purchase-coverage`, and `--max-unmatched-cash-rows`, `--strict-coverage` remains accepted as an explicit compatibility flag, and `--force-partial-coverage` provides explicit opt-out continuation with warning.
