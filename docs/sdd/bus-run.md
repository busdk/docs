...
FR-RUN-003 Pipeline expansion before execution. All pipeline tokens MUST be expanded into a flat sequence of runnable steps (prompt actions and script actions) before any step runs. Execution MUST proceed in that order, one step at a time, with stop-on-first-failure. Acceptance criteria: expansion is deterministic and testable; expanded sequence runs in order; first non-zero exit stops the run and is the process exit code.
...
- **Subcommand handlers.** Run (execute expanded sequence step by step), set (write bus-run preferences), context (print catalog), pipeline/action/script (management). Management commands that write to `.bus/run/` take the per-directory lock.

Data flow: user invokes `bus run <tokens>` or `bus run <management>`; CLI parses and resolves; for run, tokens are expanded to steps; for each prompt step, handler loads template, renders with catalog, calls bus-agent; for each script step, handler runs script with catalog as env; diagnostics to stderr, results per BusDK conventions.
...
**bus run \<token\> [\<token\> ...]** — Resolve each token (prompt action, script action, or pipeline name). Expand pipelines to a flat sequence of prompt and script actions. Execute the sequence in order; stop on first non-zero exit. When any token resolves to directory-local content, `.bus/run/` is read from the effective working directory (project root); no Git is required. If the project root does not exist or is not accessible, exit 1. Takes the per-directory lock.
...
Stdout: Reserved for deterministic command results (e.g. context KEY=VALUE output, pipeline list, action list, script list). Agent and script output may be streamed to stderr when the run is interactive or script-friendly as documented.
...
- **Pipeline expansion errors:** Unknown token, cycle, expansion limit exceeded, invalid YAML or invalid preference JSON → exit 2 before any agent or script run.
...
- **Management command tests.** Pipeline/action/script set and unset, list output determinism, ambiguity detection (same NAME in two definition types → exit 2), and preference read/write via bus-preferences.
...
- **Pipeline:** A named sequence of tokens (each token resolves to a prompt action, script action, or another pipeline) stored in `.bus/run/<NAME>.yml` (directory-local) or in preference `bus-run.pipeline.<name>`. Pipelines expand to a flat sequence of prompt and script actions only.
...
- **Runnable step:** After expansion, each step is either a prompt action or a script action. Execution runs these in order with stop-on-first-failure.
