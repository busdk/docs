...
FR-RUN-001 CLI integration and naming. The binary MUST be named `bus-run` and MUST be invoked through the dispatcher as `bus run [global-flags] <operation> [operation ...]`. Operations are: one or more runnable tokens (user-defined prompt action, script action, or pipeline name), the subcommand **retry**, and the management subcommands **set**, **context**, **list**, **pipeline**, **action**, and **script**. Acceptance criteria: the dispatcher routes `bus run <token>`, `bus run <token> <token>`, `bus run retry`, `bus run list`, and `bus run pipeline list` (etc.) to the same binary.
...
FR-RUN-003b Retry with fallback and exponential backoff. The implementation MUST provide `bus run retry --on-fail|-f <TOKEN>[,<TOKEN>...] [--attempts N] <workflow-token>...`. The tool MUST resolve and normalize fallback and workflow sequences using the same token-resolution and duplicate-merge logic as `bus run <token...>`. `--attempts` defines the number of retries after the first attempt (default `1`, minimum `0`). On workflow failure and while attempts remain, the implementation MUST run the fallback sequence, wait exponentially (`1s`, `2s`, `4s`, ...) and then rerun the workflow. If the final attempt succeeds, command exits `0`; if all attempts fail, command exits with the last workflow exit code. Acceptance criteria: `--attempts 0` runs the workflow once with no fallback, retries are bounded by `attempts`, fallback is executed for each failure before each retry, and backoff is exponential.
...
FR-RUN-007 Dependence on bus-agent. The implementation MUST depend on the bus-agent Go library for all agent invocations. Bus Run MUST use bus-agent’s runner interface, template renderer, runtime detection, and installation references. Model, output format, and timeout are configurable via flags or environment; resolution order MUST follow a documented bus-run-specific order (e.g. `--agent`, then `BUS_RUN_AGENT`, then `BUS_AGENT`, then selector preference `bus-run.agent_for.<selector>`, then `bus-run.agent`, then `bus-agent.runtime`, then first available). Acceptance criteria: bus-run imports and uses bus-agent for every agent run; run-config is configurable and resolution order is documented.

FR-RUN-007a Agent output and disclosure on each step. When the tool executes an agent-invoking step (prompt action), it MUST print to stderr, at the start of that step and before streaming or capturing agent output, which internal agent runtime and which model are in use. The agent's output (human-readable; e.g. NDJSON formatted to text per bus-agent's contract) MUST be written to stdout so that it can be piped (e.g. `bus run open-tasks | do-something`); the tool MUST NOT suppress or discard agent output. Acceptance criteria: at the start of each prompt-action step the tool writes one line to stderr identifying runtime and model; agent output is streamed or written to stdout in a human-readable form; no code path discards or hides agent output; piping action output to another command works as intended.
...
FR-RUN-019 Help and disclosure. When the user requests help (e.g. `bus run --help` or `bus run -h`), the tool MUST print to stdout, without running any agent, script, or other execution: (1) a concise usage line and global flags; (2) a statement that Bus Run has no built-in operations or pipelines — runnable tokens are user-defined only: prompt actions (`.bus/run/<name>.txt`), script actions (`.bus/run/<name>.sh`, `.bat`, `.ps1`), and pipelines (`.bus/run/<name>.yml` or preference `bus-run.pipeline.<name>`); (3) direct the user to **bus run list**, **bus run pipeline list**, **bus run action list**, and **bus run script list** to discover available tokens. Help MUST be producible without requiring the project root to exist so that users can see the command surface from any working directory. Acceptance criteria: `bus run --help` shows usage, explains user-defined-only model, and directs to list commands; no execution.

FR-RUN-020 Unified listing without execution. The implementation MUST provide a command **bus run list** that prints a deterministic, parseable listing of every runnable token available in the current context, with no execution of any agent, script, or other runnable step. When the effective working directory does not exist or is not accessible, **bus run list** MUST exit with code 1 and a clear diagnostic (same as **bus run context**). When the project root exists and is accessible, **bus run list** MUST list: directory-local pipelines (source path and normalized expanded step sequence), directory-local prompt actions (source path `.bus/run/<NAME>.txt`), directory-local script actions (source path and platform variants), and preference pipelines (source key and normalized expanded step sequence). Each entry MUST include token name, type (pipeline | action prompt | action script), a short description or source identifier, and for pipelines the normalized expanded step sequence. Output format MUST be stable, line-oriented or otherwise parseable, and documented. **list** is a management operation that does not participate in runnable token expansion and does not acquire the working-directory lock. Acceptance criteria: `bus run list` when workdir inaccessible exits 1; when workdir accessible shows all user-defined pipelines, prompt actions, and script actions with source and expansion where applicable; no execution; format is deterministic and documented.
...
- **CLI layer.** Parses global flags (including BusDK-standard `-C`, `-o`, `-v`, `-q`, `--help`, `--version`) and the operation: one or more runnable tokens (for `bus run token [token ...]`), **retry**, or a management subcommand (set, context, list, pipeline, action, script). When `--help` is requested, the CLI prints usage, the user-defined-only model, and directs to list commands (FR-RUN-019) without running any handler. When the operation is **list**, the CLI prints the unified catalog of runnable tokens (directory-local and preference pipelines, prompt actions, script actions) without execution (FR-RUN-020); list requires the project root and exits 1 when it is inaccessible. For run and retry, tokens are resolved and pipelines expanded to a flat sequence of prompt and script actions before any step runs, then normalized by merging repeated step names so each name appears once in first-appearance order. The CLI resolves the effective working directory (project root) and, for agent-invoking steps, the agent runtime via bus-run resolution order (e.g. `--agent`, `BUS_RUN_AGENT`, `BUS_AGENT`, `bus-run.agent`, `bus-agent.runtime`, first available). For run/retry and directory-writing management, the CLI acquires the per-directory lock before proceeding.
...
Data flow: user invokes `bus run <tokens>`, `bus run retry`, or `bus run <management>`; CLI parses and resolves; for run and retry, tokens are expanded and normalized to final steps; for retry, fallback steps are resolved with the same rules and run before each retry. For each prompt step, handler loads template, renders with catalog, calls bus-agent; bus-agent's output is streamed or written to stdout so it can be piped (FR-RUN-007a); for each script step, handler runs script with catalog as env and script stdout is passed through to process stdout; all action output (prompt-action and script-action) goes to stdout; diagnostics, progress, and runtime/model disclosure go to stderr; management commands (list, context, pipeline list, etc.) write deterministic results to stdout. `pipeline preview` follows the same parse/resolve/expand/normalize path and emits the final step list without execution.
...
**Invocation.** `bus run [global-flags] ( <token> [<token> ...] | retry | set | context | list | pipeline | action | script )`. Global `--help` (e.g. `bus run --help`) MUST show usage, state that runnable tokens are user-defined only (no built-in operations or pipelines), and direct the user to `bus run list`, `bus run pipeline list`, `bus run action list`, and `bus run script list` for discovery (FR-RUN-019).

**bus run retry --on-fail|-f <TOKEN>[,<TOKEN>...] [--attempts N] <workflow-token>...** — Execute one or more workflow tokens and on failure execute fallback tokens and retry with exponential backoff up to `N` additional times. Workflow and fallback token lists are resolved and normalized before execution as in `bus run` and must each contain at least one runnable step.
...
**bus run set agent \<runtime\>** — Set `bus-run.agent` via bus-preferences. `<runtime>` must be one of `cursor`, `codex`, `codex:local`, `gemini`, `claude`. Invalid value → exit 2.
...
**bus run list** — Print every runnable token available in the current context and what each executes, without running any agent, script, or other runnable step (FR-RUN-020). When the effective working directory does not exist or is not accessible, exit 1 with a clear diagnostic (same as context). When the project root is accessible, output includes directory-local pipelines (source path and normalized expanded step sequence), directory-local prompt actions (`.bus/run/<NAME>.txt`), directory-local script actions (source and platform variants), and preference pipelines (source key and expanded step sequence). Each entry includes token name, type (pipeline | action prompt | action script), short description or source, and for pipelines the normalized expanded step sequence. Output format is stable and parseable. Does not take the lock.
...
Stdout is reserved for (1) deterministic command results from management commands (list, context KEY=VALUE output, pipeline list, pipeline preview output, action list, script list), and (2) all runnable-step output so that piping works. Custom script action stdout, custom prompt action (agent) output, and any built-in action output MUST be written to stdout. This allows invocations such as `bus run open-tasks | do-something` or `bus run open-tasks` to produce pipeable output from the action itself.

Stderr: All diagnostics, progress, and the single line at the start of each agent-invoking step that identifies the internal agent runtime and model (FR-RUN-007a) MUST go to stderr. The tool MUST NOT write action result content to stderr so that stdout remains the sole stream for action output and can be piped reliably.
