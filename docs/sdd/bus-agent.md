...
FR-AGT-003 Supported runtimes. The implementation MUST include at least four backends: Cursor CLI, Codex CLI, Gemini CLI, and Claude CLI. Runtime selection MUST also accept `codex:local` as a runtime token that maps to the Codex backend in local mode (`--oss`). Acceptance criteria: all four backends can be selected; supported runtime tokens resolve deterministically; each backend implements the same interface; each backend has documented executable name(s) used for detection.
...
FR-AGT-012 Set preference subcommands (CLI). If the CLI is implemented, it MUST provide a dedicated set subcommand for each bus-agent preference key, using the [bus-preferences](./bus-preferences) Go library (no shell-out to `bus preferences`). The subcommands MUST be: `bus agent set runtime <runtime>`, `bus agent set model <value>`, `bus agent set model-reasoning-effort <minimal|low|medium|high|xhigh>`, `bus agent set model-verbosity <low|medium|high>`, `bus agent set model-reasoning-effort-for-model <model> <minimal|low|medium|high|xhigh>`, `bus agent set model-verbosity-for-model <model> <low|medium|high>`, `bus agent set output-format <value>`, `bus agent set timeout <duration>`, each writing the corresponding key in Data Design (`bus-agent.runtime`, `bus-agent.model`, `bus-agent.model_reasoning_effort`, `bus-agent.model_verbosity`, `bus-agent.model_reasoning_effort_for_model.<model>`, `bus-agent.model_verbosity_for_model.<model>`, `bus-agent.output_format`, `bus-agent.timeout`). Invalid value for a given subcommand MUST yield exit 2. Acceptance criteria: running `bus agent set runtime gemini` persists `bus-agent.runtime`; a subsequent run without overrides uses that default; invalid value yields exit 2.
...
* Runtime ID (backend IDs: `cursor`, `codex`, `gemini`, `claude`; accepted runtime tokens also include `codex:local` for Codex local mode)
...
* **`bus agent set runtime <runtime>`** — Set `bus-agent.runtime` (e.g. `cursor`, `codex:local`, `gemini`) via the [bus-preferences](./bus-preferences) Go library. **`bus agent set model <value>`** — Set `bus-agent.model` (default when unset: `auto`). **`bus agent set model-reasoning-effort <minimal|low|medium|high|xhigh>`** — Set `bus-agent.model_reasoning_effort`. **`bus agent set model-verbosity <low|medium|high>`** — Set `bus-agent.model_verbosity`. **`bus agent set model-reasoning-effort-for-model <model> <minimal|low|medium|high|xhigh>`** — Set `bus-agent.model_reasoning_effort_for_model.<model>`. **`bus agent set model-verbosity-for-model <model> <low|medium|high>`** — Set `bus-agent.model_verbosity_for_model.<model>`. **`bus agent set output-format <ndjson|text>`** — Set `bus-agent.output_format` (default when unset: `text`). **`bus agent set timeout <duration>`** — Set `bus-agent.timeout` (e.g. `60m`). Each uses the bus-preferences library directly (no shell-out to `bus preferences`). Invalid value yields exit 2.
...
**Environment variables for runtime selection.** `BUS_AGENT` is the session preference for the default agent when using the bus-agent CLI or when no module-specific override applies; its value MUST be one of `cursor`, `codex`, `codex:local`, `gemini`, or `claude`. When the caller is bus-dev, the library also consults `BUS_DEV_AGENT` (bus-dev-only session preference) before `BUS_AGENT`; see FR-AGT-005. These variables are read by the library during resolution; the library does not set them.

**Preference keys.** The following preference keys under the `bus-agent` namespace are used:

| Key | Description |
|-----|-------------|
| `bus-agent.runtime` | Default agent runtime when no per-call or session override is set (e.g. `cursor`, `codex:local`, `gemini`). |
...
| `bus-agent.model_reasoning_effort` | Default model reasoning effort for runtimes that support it (Codex values: `minimal`, `low`, `medium`, `high`, `xhigh`). Overridable by `BUS_AGENT_MODEL_REASONING_EFFORT`. |
| `bus-agent.model_verbosity` | Default model verbosity for runtimes that support it (Codex values: `low`, `medium`, `high`). Overridable by `BUS_AGENT_MODEL_VERBOSITY`. |
| `bus-agent.model_reasoning_effort_for_model.<model>` | Per-model model reasoning effort override, applied when resolved model equals `<model>`. |
| `bus-agent.model_verbosity_for_model.<model>` | Per-model model verbosity override, applied when resolved model equals `<model>`. |
...
* **BUS_AGENT:** Environment variable for session default agent (value: `cursor`, `codex`, `codex:local`, `gemini`, or `claude`). Consulted during resolution when no explicit per-call selection is given; used by both bus-agent CLI and bus-dev (after BUS_DEV_AGENT when caller is bus-dev). See FR-AGT-005.
