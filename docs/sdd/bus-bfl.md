...
Parity-level behavior: when formula evaluation is enabled for workbook read, output MUST contain the evaluated result (e.g. numeric) for formula-driven cells, not the formula source text, unless the consumer opts in to include formula source. Locale options (`--decimal-sep`, `--thousands-sep`) MUST be applied so that both formula literal parsing and locale-formatted cell values are normalized deterministically (e.g. a cell value `1 234,56` with space thousands and comma decimal becomes `1234.56` in output when those separators are configured). Verification steps for formula output and locale normalization are given in the [workbook formula delegation doc](../modules/bus-bfl-workbook-formula-delegation#verification) and in the [bus-data SDD implementation status](../sdd/bus-data#implementation-status-workbook-read).
