...
NFR-REP-002 Decimal-safe money arithmetic. All report money calculations (trial-balance values, statement totals, parity deltas, and thresholds) MUST use decimal-safe arithmetic and MUST NOT use binary floating-point for business calculations. Acceptance criteria: calculations use exact decimal-safe representations (scaled cents or exact decimal/rational types), rounding rules are explicit, and monetary outputs remain deterministic across runs.
...
Source import parity report (suggested extension). Add a report in bus-reports that emits source import parity as a review/CI artifact: counts and sums by dataset and period in machine-friendly format (tsv/csv), complementing `bus validate parity`. Optional thresholds for CI could be documented. When adopted, the SDD may add a new report subcommand and output contract; module docs will be required for report output format and any CI threshold options.
...
Built-in Finnish layouts (e.g. `pma-full`, `kpa-full` or equivalent `fi-*` layout ids) execute successfully for balance-sheet and profit-and-loss and emit Finnish section labels (e.g. Pysyvät vastaavat, LIIKEVAIHTO). Output is section-level summaries, not full line-by-line parity with a source TASE or tuloslaskelma. Still to do: provide full line-by-line parity — either a built-in Finnish full layout or a robust custom YAML/JSON layout that maps accounts to report lines with the same Finnish labels and hierarchy as the source; output CSV/PDF would then match the same line structure as the original for comparison and filing. A project-maintained parity table may be used to validate or document the target structure. No SDD change is required (FR-REP-004 already covers configurable layout). When full parity is delivered, documentation is required for the Finnish full layout or custom layout format and for the parity mapping (field names, line ids, hierarchy). Until that path is complete, parity work for comparison and filing may rely on custom layout files and account-mapping refinement.
