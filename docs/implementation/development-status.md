...
| [invoices](../modules/bus-invoices#development-state) | 80% — record-invoices and postings step completable; pdf not verified. | First-class profile-driven ERP invoice import; help alignment (add --due-date optional, init workspace root); optional attachment_id validation (FR-005). | [pdf](../modules/bus-pdf) for `bus invoices pdf`. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 60% — match, allocate, and list are verified; when a valid matches dataset exists, match works as expected; first-class proposal generation and batch apply are not implemented. | Add `propose` and `apply` commands (confidence/reasons, dry-run, idempotent re-apply), plus deterministic coverage artifacts for parity and gap checks. | None known; first-class propose/apply not yet implemented. |
| [validate](../modules/bus-validate#development-state) | 50% — Run and schema/invariant checks verified; user can complete pre-close validate step; format and empty stdout would complete contract. | format text/tsv; empty stdout and --output no-op; parity and journal-gap checks with thresholds and CI exit; audit and closed-period checks. | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Trial-balance, general-ledger, profit-and-loss, balance-sheet, account-ledger with text/csv/json/markdown/kpa/pma/pdf, built-in statutory layouts (kpa, pma, kpa-full) and layout-file selection verified by e2e; user can complete report step. | Add non-opening journal coverage report for migration gap checks. | None known. |
...
| [validate](../modules/bus-validate#development-state) | 50% — Workspace validation before close/filing verified; audit and closed-period would strengthen. | format text/tsv; empty stdout; parity and journal-gap checks; audit and closed-period checks. | None known. |
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Reports, traceability (basis in JSON), statutory layouts and PDF verified by e2e; user can produce statement outputs for compliance. | Add non-opening journal coverage report for migration gap checks. | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Trial balance and ledgers as audit evidence; statutory layouts and PDF verified by e2e; user can produce evidence-pack reports. | Add non-opening journal coverage report for migration gap checks. | None known. |
| [validate](../modules/bus-validate#development-state) | 50% — Workspace validation before evidence pack verified; same gaps as accounting/compliance. | format text/tsv; empty stdout; parity and journal-gap checks; audit and closed-period. | None known. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 60% — reconciliation evidence path exists through match, allocate, and list; when a valid matches dataset exists, match works as expected; first-class proposal generation and batch apply are not implemented. | Add `propose` and `apply` commands (confidence/reasons, dry-run, idempotent re-apply), plus deterministic coverage artifacts for parity and gap checks. | None known; first-class propose/apply not yet implemented. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 60% — payroll bank reconciliation works with match, allocate, and list; when a valid matches dataset exists, match works as expected; first-class proposal generation and batch apply are not implemented. | Add `propose` and `apply` commands (confidence/reasons, dry-run, idempotent re-apply), plus deterministic coverage artifacts for parity and gap checks. | None known; first-class propose/apply not yet implemented. |
