...
| [journal](../modules/bus-journal#development-state) | 70% (Broadly usable) – user can init, add balanced postings (by code or name), and run balance; NFR-JRN-001 and audit columns verified; interactive add not verified. | Optional: interactive add. | [period](../modules/bus-period) writing closed-period file for full workflow. |
...
| Module | Readiness | Biggest next | Biggest blocker |
|--------|-----------|--------------|-----------------|
| [dev](../modules/bus-dev#development-state) | 60% (Stable) – e2e proves init scaffold, flags, set, per-directory lock, context, pipeline list builtin, post-init restrictions; unit tests prove plan/work/spec/e2e/stage/commit and pipeline expansion with stub agent. | action/script generate once [agent](../modules/bus-agent) exposes stdout API; AGENTS.md assert; README. | None known. |
...
| [entities](../modules/bus-entities#development-state) | 50% (Primary journey) – init, add, list and `--id`/`--name` aliases verified by e2e; user can define party references for employees. | None in PLAN; optional SDD follow-ups. | None known. |
