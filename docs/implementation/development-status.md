...
| [invoices](../modules/bus-invoices#development-state) | 80% (Broadly usable) – record-invoices and postings journey completable; init, add, validate (totals and VAT), list with all filters, postings verified by e2e and unit tests; pdf not verified in e2e. | None in PLAN.md; optional postings help, attachment_id validation (FR-005). | [pdf](../modules/bus-pdf) for `bus invoices pdf`. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 70% (Broadly usable) – match, allocate, list and global flags verified by unit and e2e; user can complete reconciliation step. | Tests for amount/sum mismatch, already-reconciled, missing ref (PLAN.md); bank path from [bank](../modules/bus-bank). | None known. |
...
| [vat](../modules/bus-vat#development-state) | 80% (Broadly usable) – init (incl. --dry-run), validate (incl. rate check), report/export from invoice or journal with source_refs, index update, closed-period/--force, path API verified by e2e and unit tests; user can complete close-step VAT from either source. | Period transitions and non-standard period lengths; validate --source journal (PLAN.md). | None known. |
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Close-step reports verified by e2e; trial-balance, general-ledger, profit-and-loss, balance-sheet, account-ledger with text/csv/json/markdown, KPA/PMA, TASE/tuloslaskelma PDF; `--layout` file with custom labels verified. | FR-REP-004 account→line mapping in layout (built-in or layout file). | None known. |
| [pdf](../modules/bus-pdf#development-state) | 70% — Render step verified; user can complete PDF output; `bus invoices pdf` not yet in [invoices](../modules/bus-invoices). | None in PLAN.md. | None known. |
...
| [invoices](../modules/bus-invoices#development-state) | 80% (Broadly usable) – create, validate, list, postings verified; pdf step blocked by [pdf](../modules/bus-pdf). | None in PLAN.md; optional postings help, attachment_id validation (FR-005). | [pdf](../modules/bus-pdf) for `bus invoices pdf`. |
| [pdf](../modules/bus-pdf#development-state) | 70% — PDF generation step verified; `bus invoices pdf` not yet in [invoices](../modules/bus-invoices). | None in PLAN.md. | None known. |
...
| [data](../modules/bus-data#development-state) | 80% (Stable) – package, resource add/remove/rename, schema key set, foreign-key add/remove, field remove/rename/set-format/set-constraints/set-missing-values, table list/read/workbook, row mutate, formula verified by e2e/unit tests; backend for [api](../modules/bus-api) and [sheets](../modules/bus-sheets). Workbook command not yet in SDD/CLI reference. | Document table workbook in SDD and CLI reference (KD-DAT-005). | None known. |
...
| [vat](../modules/bus-vat#development-state) | 80% (Broadly usable) – VAT report and export from invoice or journal with voucher/invoice refs in output verified; index update, closed-period/--force, rate validation verified. | Period transitions and non-standard period lengths; validate --source journal (PLAN.md). | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Reports, traceability (basis in JSON), KPA/PMA, TASE/tuloslaskelma PDF verified by e2e; `--layout` file with custom labels verified. | FR-REP-004 account→line mapping in layout. | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Trial balance and ledgers as audit evidence; KPA/PMA and TASE/tuloslaskelma PDF verified by e2e; `--layout` file with custom labels verified. | FR-REP-004 account→line mapping in layout. | None known. |
| [validate](../modules/bus-validate#development-state) | 50% — Workspace validation before evidence pack verified; format and empty stdout same gaps as accounting. | format text/tsv; empty stdout; audit and closed-period. | None known. |
...
| [invoices](../modules/bus-invoices#development-state) | 80% (Broadly usable) – invoice evidence-pack baseline verified; init, add, validate, list, postings verified; pdf not in journey. | None in PLAN.md; optional postings help, attachment_id validation (FR-005). | [pdf](../modules/bus-pdf) for `bus invoices pdf`. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 70% (Broadly usable) – match, allocate, list verified; reconciliation evidence path completable. | Tests for amount/sum mismatch, already-reconciled, missing ref (PLAN.md); bank path from [bank](../modules/bus-bank). | None known. |
...
| [assets](../modules/bus-assets#development-state) | 70% (Stable) – init, add, validate, schedule, depreciate, dispose, post verified by e2e and unit tests; user can complete asset lifecycle and produce postings for evidence pack; workspace-root layout verified. | Partial-init failure test (FR-INIT-004); path accessors (NFR-AST-002). | None known. |
...
| Module | Readiness | Biggest next | Biggest blocker |
|--------|-----------|--------------|-----------------|
| [dev](../modules/bus-dev#development-state) | 60% (Stable) – init, set, flags, context, pipeline list/preview verified by e2e; plan/work/spec/e2e/stage/commit and repo-local pipeline override verified by unit tests with stub agent; user can complete init and run workflow steps. | Submodule commit test (PLAN.md); action/script generate once [agent](../modules/bus-agent#development-state) exposes stdout API. | None known. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 70% (Broadly usable) – match, allocate, list verified; payroll bank reconciliation step completable. | Tests for amount/sum mismatch, already-reconciled, missing ref (PLAN.md); bank path from [bank](../modules/bus-bank). | None known. |
...
| Module | Readiness | Biggest next | Biggest blocker |
|--------|-----------|--------------|-----------------|
| [replay](../modules/bus-replay#development-state) | 50% (Primary journey) — export (empty + populated) and render verified; apply executes via dispatcher; roundtrip and idempotency when bus on PATH (e2e); full export plan not yet implemented. | In-process executor for apply; complete export plan (config set, attachment registrations, VAT/reports). | None known. |
...
**replay:** 50% overall. Value promise: export workspace to a deterministic replay log and apply it into a clean workspace for migration and parity. Export (empty and populated), render, and apply (via dispatcher) with roundtrip and idempotency verified when bus on PATH. No new use case doc needed — operator/automation tool, not a step in a documented end-user workflow.
