...
| [reconcile](../modules/bus-reconcile#development-state) | 80% (Broadly usable) — reconciliation step completable; match, allocate, list and validation failures verified by unit and e2e. | Bank path from [bank](../modules/bus-bank) Go library; reject invoice status canceled in match (PLAN.md). | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Close-step reports verified by e2e; trial-balance, general-ledger, profit-and-loss, balance-sheet, account-ledger with text/csv/json/markdown, KPA/PMA, TASE/tuloslaskelma PDF; `--layout` file with custom labels and account_mapping verified. | None in PLAN.md. | None known. |
| [pdf](../modules/bus-pdf#development-state) | 70% — Render step verified; user can produce PDF from file/stdin; `bus invoices pdf` not yet in [invoices](../modules/bus-invoices). | None in PLAN.md. | None known. |
...
| [pdf](../modules/bus-pdf#development-state) | 70% — PDF generation step verified; user can produce PDF from file/stdin; `bus invoices pdf` not yet in [invoices](../modules/bus-invoices). | None in PLAN.md. | None known. |
...
| [api](../modules/bus-api#development-state) | 80% — Resources, package, row CRUD, resource add/rename/remove, workspace and single-resource validate, schema GET/PATCH and fine-grained fields/primaryKey, events, read-only and serve flags verified by e2e and unit tests; stable key ordering for single-resource validate success pending (PLAN.md). | Stable JSON key ordering for POST /resources/{name}/validate success (PLAN.md). | None known. |
| [data](../modules/bus-data#development-state) | 80% (Stable) — user can complete package/resource lifecycle, schema evolution, table and workbook-style read (cell/range, --header, --anchor-col/--anchor-row, --decimal-sep, --formula), and row mutate; workbook command doc in SDD/CLI reference pending. | Document table workbook in SDD and CLI reference (KD-DAT-005); add or alias schema field set-type (PLAN.md). | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Reports, traceability (basis in JSON), KPA/PMA, TASE/tuloslaskelma PDF verified by e2e; `--layout` file with account_mapping verified. | None in PLAN.md. | None known. |
...
| [reports](../modules/bus-reports#development-state) | 90% (Broadly usable) – Trial balance and ledgers as audit evidence; KPA/PMA and TASE/tuloslaskelma PDF verified by e2e; `--layout` file with account_mapping verified. | None in PLAN.md. | None known. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 80% (Broadly usable) — reconciliation evidence path verified; match, allocate, list and validation failures verified. | Bank path from [bank](../modules/bus-bank) Go library; reject invoice status canceled in match (PLAN.md). | None known. |
...
| [assets](../modules/bus-assets#development-state) | 70% (Stable) – init (incl. FR-INIT-004), add, validate, schedule, depreciate, dispose, post and path accessors verified; user can complete asset lifecycle and produce postings for evidence pack; FR-AST-003, FR-AST-004 and dispose required-args not test-covered. | FR-AST-003 cap at disposal; require --gain-account/--loss-account on dispose; FR-AST-004 test. | None known. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 80% (Broadly usable) — payroll bank reconciliation step completable; match, allocate, list and validation failures verified. | Bank path from [bank](../modules/bus-bank) Go library; reject invoice status canceled in match (PLAN.md). | None known. |
...
| Module | Readiness | Biggest next | Biggest blocker |
|--------|-----------|--------------|-----------------|
| [replay](../modules/bus-replay#development-state) | 90% (Broadly usable) — full accounting snapshot export and apply verified; roundtrip and idempotency when bus on PATH (e2e); in-process apply for config/accounts add. | None in PLAN.md. | None known. |
...
**replay:** 90% overall. Value promise: export workspace to a deterministic replay log and apply it into a clean workspace for migration and parity. Full accounting snapshot export (inits + accounts add, period add/state, journal add) and apply (guards, dry-run, execution; in-process for config/accounts add) verified by golden and e2e; roundtrip and idempotency when bus on PATH. No new use case doc needed — operator/automation tool, not a step in a documented end-user workflow.
