...
| [reconcile](../modules/bus-reconcile#development-state) | 60% — match, allocate, and list are verified; first-class proposal generation and batch apply are not implemented; current workspace has a bank-id lookup defect in match. | Add `propose` and `apply` commands (confidence/reasons, dry-run, idempotent re-apply), plus deterministic coverage artifacts for parity and gap checks; fix bank-id lookup defect in match. | Active bank-id lookup defect in current workspace match path. |
...
| [reconcile](../modules/bus-reconcile#development-state) | 60% — reconciliation evidence path exists through match, allocate, and list; first-class proposal generation and batch apply are not implemented; current workspace has a bank-id lookup defect in match. | Add `propose` and `apply` commands (confidence/reasons, dry-run, idempotent re-apply), plus deterministic coverage artifacts for parity and gap checks; fix bank-id lookup defect in match. | Active bank-id lookup defect in current workspace match path. |
