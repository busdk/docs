...
| [api](../modules/bus-api#development-state) | 70% — Resources, package, row CRUD, resource add/rename/remove, workspace validate, events, read-only and serve flags verified; schema endpoints and single-resource validate not covered. | e2e for schema endpoints and single-resource validate; schema field rename PATCH; OpenAPI stable key ordering. | None known. |
