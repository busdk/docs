...
Agentic workflows are separate from the model proxy. Start with [bus-agent](../modules/bus-agent) for shared runtime and host behavior, [bus-run](../modules/bus-run) for local prompts, scripts, and pipelines, [bus-task](../modules/bus-task) for durable task threads, and [bus-worker](../modules/bus-worker) for worker identity and control. Use [bus-dev](../modules/bus-dev) when the work is about module-repository development workflows. These tools can read structured workspace datasets, run the same CLI workflows as a human, and propose changes as reviewable updates to repository data. The retired [bus-work](../modules/bus-work) page documents the removed generic `bus.work.*` surface.
