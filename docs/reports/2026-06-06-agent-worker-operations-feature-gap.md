...
`bus-worker`, through the plural `bus-worker` product command, is the current
...
1. Add `bus worker evidence` for one worker id using existing workers API,
   Events replay, logs, attach, and message projection.
2. Add redaction tests for evidence, logs, attach, and support diagnostics.
3. Add `bus worker doctor` against the local service stack.
