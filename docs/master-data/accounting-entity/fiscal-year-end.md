...
`fiscal_year_end` defines when the accounting entity’s financial year ends. Bookkeeping uses the year end as a hard boundary for annual reporting and closing workflows, and it helps define which periods can be locked and which reports belong to the same financial year.
