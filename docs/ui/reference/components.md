...
This page is an overview. Use [the per-component reference](./component-reference)
for required fields, defaults, constraints, and copyable examples for every
component and runtime block.
...
Provider/session runtime should use the same [`Action`](../concepts/action),
[`Resource`](../concepts/resource), and [`Effect`](../concepts/effect) model as
the rest of the framework. A credential login submit, JSON request, file
upload, token refresh, and provider status refresh are not separate app
architectures; they are actions or effects operating on resources.
...
<p class="busdk-prev-next">
  <span class="busdk-prev-next-item busdk-prev">&larr; <a href="../architecture/rendering">Rendering model</a></span>
