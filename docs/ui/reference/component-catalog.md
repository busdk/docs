...
Unless a row explicitly says otherwise, each `Kind` is available both as a Go
`bus-ui` component/helper and as a declarative `kind` value in JSON/YAML UI
documents. Runtime blocks such as `CSSBundle`, `CLIRuntimeFlags`,
`BrowserOpen`, and `DeclarativeRenderer` are Go/command configuration surfaces;
their dedicated pages state when a snippet is host config instead of a UI
document node.
...
| `DeclarativeRenderer` | Render JSON/YAML UI document. | UI document file plus command or host output settings. | HTML, component tree, or validation diagnostics. | Schema validation and deterministic render. |
