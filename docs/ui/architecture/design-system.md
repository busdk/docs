...
- tooltips or `title` text as supplemental hints for icon-only controls.

Icon-only controls must still provide `aria-label` or equivalent text. A
tooltip or HTML `title` alone is not a reliable accessible name.
...
<p class="busdk-prev-next">
  <span class="busdk-prev-next-item busdk-prev">&larr; <a href="./">Architecture</a></span>
