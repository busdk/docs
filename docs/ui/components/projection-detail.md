...
| `evidence` | yes | array of evidence items | Evidence items use `{id,label,url,type}`. `id` is stable and unique, `url` follows the safe URL rules below, and `type` is a display/media hint such as `pdf`, `image`, `text`, or `link`. Unknown evidence IDs referenced by lines render as unavailable links and are reported through [ErrorHost](./error-host). |
...
`{ base: artifact, path: /evidence/receipt.pdf }`. External origins are rejected
unless the portal or local app host explicitly lists the origin in its artifact
URL allowlist before rendering. `javascript:`, `data:`, path traversal, and
unresolved authorization fail validation.
