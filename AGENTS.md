...
When reviewing, rewriting, or restructuring the public UI framework
documentation under `docs/docs/ui/**`, promoting a UI feature candidate, or
changing UI docs page structure, versioning, sidebar ordering, or version
directory layout, read `runbooks/ui-framework-design-review.md` first and
apply its full review contract. Core reminders: treat current pages as
editable design material; keep one source of truth per concept; review
top-down from design decisions to component pages; and review every
higher-level component as a candidate for composition from smaller Bus UI
pieces before accepting raw-code implementations.
